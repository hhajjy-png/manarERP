...
export function ExportBar({ onExcelExport, onPdfExport, loading }: Props) {
  return (
    <div className="financial-export-bar">
      <button type="button" className="export-btn excel" onClick={onExcelExport} disabled={loading}>
        <span className="material-symbols-outlined">table_view</span> Excel
      </button>
      <button type="button" className="export-btn pdf" onClick={onPdfExport} disabled={loading}>
