...
import { PAYMENT_METHOD_LABEL, type CategoryBalance, type PayableCategory, type PaymentRow } from './entitlementsShared';

/** طرق الدفع — تطابق ENUMS.leaveSettlementPaymentMethod في الخادم. */
const PAYMENT_METHODS: { value: string; labelKey: string }[] = [
  { value: 'CASH', labelKey: PAYMENT_METHOD_LABEL.CASH },
  { value: 'BANK_TRANSFER', labelKey: PAYMENT_METHOD_LABEL.BANK_TRANSFER },
  { value: 'CHEQUE', labelKey: PAYMENT_METHOD_LABEL.CHEQUE },
  { value: 'OTHER', labelKey: PAYMENT_METHOD_LABEL.OTHER },
];

interface Props {
  employeeId: number;
  category: PayableCategory;
  /** موقف الفئة كما احتسبه الخادم — للعرض قبل التأكيد فقط، والخادم يبقى الحَكَم. */
  balance: CategoryBalance;
  /** الدفعة قيد التعديل — غيابها يعني تسجيل دفعة جديدة. الفئة والموظف لا يُعدَّلان. */
...
/**
 * تسجيل دفعة مستحق فعلية.
 *
 * المستخدم يُدخل **المبلغ والتاريخ** (وطريقة الدفع ومرجعًا/ملاحظة اختياريين) — لا يُدخل
 * استحقاقًا محتسَبًا، ولا رصيدًا متبقيًا، ولا أيام إجازة مكافئة. الأرقام المعروضة أدناه
 * (المستحق/المدفوع/المتبقي بعد الدفعة) عرضٌ لما احتسبه الخادم بالفعل ومعاينة فورية فقط؛
 * التحقق النهائي — بما فيه رفض التجاوز — يقع في الخادم وحده.
 */
export default function EntitlementPaymentDialog({ employeeId, category, balance, payment, onClose, onSaved }: Props) {
  const { t } = useT();
  ...
  // عند التعديل تُستبعد الدفعة نفسها من «المدفوع سابقًا»، وإلا لصادمت نفسها فبدت كل زيادة
  // تجاوزًا. نفس القاعدة التي يطبّقها الخادم — والخادم يبقى الحَكَم النهائي.
  const paidExcludingThis = isEdit ? Math.round((balance.paid - payment!.amount) * 1000) / 1000 : balance.paid;
  const availableForThis =
    balance.entitlement !== null ? Math.round((balance.entitlement - paidExcludingThis) * 1000) / 1000 : null;
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit) {
        // الفئة لا تُرسَل عند التعديل — يقرؤها الخادم من الحركة الأصلية (ثابتة بالتصميم).
        await api.patch(`/employees/${employeeId}/entitlement-payments/${payment!.id}`, body);
      } else {
        await api.post(`/employees/${employeeId}/entitlement-payments`, { category, ...body });
      }
      onSaved();
    } catch (e) {
  ...
  return (
    <Dialog
      icon="payments"
      title={isEdit ? t('page.ent.edit_payment') : t('page.ent.record_payment')}
      subtitle={t('msg.ent.payment_dialog_subtitle')}
      size="md"
      onClose={onClose}
      footer={
        <>
    ...
    >
      ...
      {/* معاينة الموقف — قيم الخادم كما هي، لا احتساب في الواجهة سوى طرح المعاينة */}
      <div className="ent-pay-preview">
        <div className="ent-pay-preview-row">
          <span>{t('field.ent.current_entitlement')}</span>
          <span>{balance.entitlement !== null ? <PrivateAmount value={balance.entitlement} level={1} /> : '—'}</span>
        </div>
        <div className="ent-pay-preview-row">
