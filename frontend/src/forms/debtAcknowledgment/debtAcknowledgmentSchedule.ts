...
/**
 * قيم الأقساط: **دنانير صحيحة** عدا الأخير، الذي يحمل الفارق كاملًا.
 *
 * ═══ القاعدة ═══
 * قسطٌ شهريّ بكسور الفلس («41.667 د.ك») لا يُدفع ولا يُحوَّل ولا يُقيَّد في دفتر:
 * فقرار مالك المنتج أن يكون القسط العادي **دينارًا صحيحًا**، وأن يستوعب القسط الأخير
 * وحده كل ما تبقّى.
 *
 *   القسط العادي = round(الرصيد ÷ العدد)   إلى أقرب دينار
 *   القسط الأخير = الرصيد − (العادي × (العدد − 1))
 *
 * مثال 500 ÷ 12: العادي `42` (عن 41.667)، فالأحد عشر الأولى `42.000` والأخير
 * `38.000` — والمجموع `500.000` بالضبط. ومثال 1000 ÷ 12: `83.000 × 11` ثم `87.000`.
 *
 * ═══ الثابت الذي لا يُخرَق ═══
 * `sum(amounts) === roundMoney(debtAmount)` بالضبط — لا فلس يضيع ولا فلس يُخترع.
 * والقسط الأخير يبقى **موجبًا**: لا صفر ولا سالب.
 *
 * ويُحمَّل الفارق على الأخير عمدًا: هو الأبعد زمنيًا، وتعديله لا يغيّر قسطًا وقّع
 * عليه المدين ودفعه فعلًا.
 *
 * ═══ حين لا يستقيم الدينار الصحيح ═══
 * التقريب إلى **أقرب** دينار قد يرفع القسط العادي فوق نصيبه، فيبتلع الأخير كلّه أو
 * يتجاوزه: `22 ÷ 12` يعطي عاديًّا `2`، و`2 × 11 = 22`، فلا يبقى للأخير شيء. عندها
 * تنزل الدالة إلى **الدينار الأدنى** (`floor`)، وهو لا يتجاوز نصيبه أبدًا فيبقى
 * للأخير موجبٌ حتمًا: `1 × 11` ثم `11`.
 *
 * ويبقى حدٌّ لا حيلة فيه: **رصيدٌ أصغر من عدد الأقساط** (10 د.ك على 12 قسطًا). لا
 * وجود لاثني عشر قسطًا صحيحًا موجبًا مجموعها عشرة — الحساب نفسه يمنعه، لا التنفيذ.
 * فترجع الدالة هناك إلى القسمة بثلاث خانات (السلوك السابق)، محافظةً على الثابتين
 * الأهمّ: المجموع مساوٍ بالضبط، ولا قسط صفر ولا سالب. وهو أقرب ما يمكن إلى القاعدة
 * حين تستحيل حرفيًّا.
 */
export function calculateInstallmentAmounts(debtAmount: number, count: number): number[] {
  ...
  // قسطٌ واحد: لا «عادي» ولا «أخير» — هو الرصيد كلّه بفلوسه إن كانت له فلوس.
  if (count === 1) return [total];

  const spread = (regular: number) => ({
    regular,
    last: roundMoney(total - regular * (count - 1)),
  });

  // 1. أقرب دينار.
  let split = spread(Math.round(total / count));
  // 2. ابتلع التقريبُ القسطَ الأخير ⇒ انزل إلى الدينار الأدنى، فهو لا يتجاوز النصيب.
  if (split.last <= 0) split = spread(Math.floor(total / count));
  // 3. حتى الدينار الأدنى صفر ⇒ الرصيد أصغر من عدد الأقساط: الدينار الصحيح مستحيل.
  if (split.regular <= 0 || split.last <= 0) {
    const base = roundMoney(total / count);
    const head = Array.from({ length: count - 1 }, () => base);
    return [...head, roundMoney(total - sumMoney(head))];
  }

  return [...Array.from({ length: count - 1 }, () => split.regular), split.last];
}
