 ...
 * القديمة. لا HTML بديل، ولا قالب معاينة، ولا إعادة كتابة
...
export interface AccurateFormPreviewOptions {
  ...
  /**
   * لغة المستند المُركَّب — **إلزامية، بلا افتراضي**. كانت اختيارية بافتراضي `'ar'`
   * صامت، فأغفلها معظم المستدعين وظلّت المعاينة الدقيقة تُركَّب بالعربية دائمًا
   * حتى للنماذج التي بدَّلها المستخدم إلى الإنجليزية — بينما «حفظ PDF» (الذي يقرأ
   * `lang` الحقيقية من كل نموذج) كان يخرج صحيحًا. الإلزام هنا **يمنع** تكرار هذا
   * الصنف من الأخطاء مستقبلًا بفحص النوع وقت البناء، لا بتخمين داخل هذا المحرك —
   * كل مستدعٍ يمرّر حالته الحقيقية (أو `'ar'` صراحةً للنماذج التي لا تملك مبدّل
   * لغة أصلًا، كعقد العمل وقسيمة الراتب — نفس القيمة التي كانت ضمنية من قبل،
   * الآن مُعلَنة).
   */
  lang: 'ar' | 'en';
