...
export function errorHandler(
  ...
  if (err instanceof AppError) {
    ...
    if (err.code === 'P2002') {
      ...
      // جدول أو عمود غير موجود = مخطط قاعدة البيانات غير مُهاجَر، وهو خلل في الخادم
      // لا في الطلب. تصنيفه 400 كان يُظهر «تعذّر تطبيق الترحيل» على هيئة رفض تحقّق،
      // فيبحث المشخِّص في مخططات Zod عن سبب غير موجود فيها. 500 يوجّهه إلى الخادم،
      // ويُدرَج الخطأ كاملًا في السجل عبر الفرع أدناه.
      statusCode = 500;
      message = 'مخطط قاعدة البيانات غير مكتمل — لم تُطبَّق ترحيلات مُعلّقة';
      details = { code: err.code, target: err.meta?.table ?? err.meta?.column };
    } else {
      statusCode = 400;
