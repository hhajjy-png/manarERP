...
export interface ReconciliationWorkspace {
  ...
  totalRows:       number;
  /**
   * إجماليات **الملف** كما وردت فيه — تشمل الصفوف التي تُخُطِّيت كمكررات، فلا تصلح
   * إجماليًا لما تعرضه المساحة. تُبقى للعرض الصريح بجانب إجماليات المساحة فقط.
   */
  fileDebits:      number;
  fileCredits:     number;
  /**
   * إجماليات **الصفوف المخزَّنة فعليًا** لهذا الاستيراد (ما دخل قاعدة البيانات).
   * هذه وحدها ما يطابق قائمة العمليات المعروضة. في الاستيراد التزايدي عالي التكرار
   * تختلف عن إجماليات الملف اختلافًا كبيرًا — 60 صفًا في الملف بثلاثة صفوف جديدة
   * كانت تعرض إجمالي الستين.
   */
  storedDebits:    number;
  storedCredits:   number;
  /** عدد الصفوف المخزَّنة فعليًا لهذا الاستيراد (بلا أي فلتر). */
  storedRows:      number;
  unmatched:       number;
  matched:         number;
  ignored:         number;
