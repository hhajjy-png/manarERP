...
/**
 * ⚠️ قرار مؤجَّل لمالك المنتج — صفوف `POTENTIAL_DUPLICATE` داخل هذه الإجماليات.
 *
 * تدقيق 2026-08-22 لاحظ أن أرصدة الحسابات تشمل الصفوف المعلَّمة تكرارًا محتملًا.
 * فحص التعريف المرجعي في الوحدة (`dedupDetector.ts`، المرحلة 4) أظهر أن دلالتها
 * **«غير محسوم» بالتعريف**: تطابق في التاريخ والمبلغ ووصف مشابه، **بلا** تطابق في
 * `transactionId` ولا `reference` ولا البصمة. لذلك يُدرجها الاستيراد عمدًا بحالة
 * `REVIEW` مع تحذير، بدل أن يتخطّاها أو يعتمدها.
 *
 * فالخياران كلاهما يكذب في حالة ما:
 *   • استبعادها ⇒ يُخفي مالًا حقيقيًا إن كانت حركتين متطابقتين في اليوم نفسه
 *     (دفعتا وقود متساويتان مثلًا — حالة واردة عمليًا).
 *   • إبقاؤها  ⇒ يضخّم الرصيد إن كانت أثر إعادة استيراد.
 *
 * لا يوجد في البيانات ما يحسم أيّهما، فلم تُغيَّر الأرقام هنا. القرار يحتاج سياسة
 * عمل صريحة: هل يُعتمد الرصيد قبل مراجعة الصفوف المعلَّمة أم بعدها؟ سُجِّل في تقرير
 * الحزمة الرابعة كبند قرار منتج. **لا تُعدَّل هذه الإجماليات بلا ذلك القرار.**
 */
