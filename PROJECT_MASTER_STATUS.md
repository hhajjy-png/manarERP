...
| **Current HEAD** | `b875f9eb` — merge of `feature/production-release-2026.5.4` (**Production Release 2026.5.4**. Packages a new self-contained Windows installer carrying **Comprehensive Reports — Monthly Employee Entitlements Report Pack v1**: «تقرير مستحقات الموظفين الشهرية» added **inside** the Comprehensive Reports centre with no standalone page, sourced exclusively from the Employee Monthly Entitlements module and reading no payroll, journal entry or general-ledger row. Every amount is read from stored per-month totals and the employee snapshot written at save time, so a historical statement reports what was actually approved; no calculation logic in that module was touched. 16 columns, final totals row, 7 KPIs, 2 analytical sections aggregated from the displayed rows themselves. Earnings split into bonuses and allowances with “other” derived by subtraction from the stored total, so the row equation always closes — verified over all 395 statements in the development database with zero mismatches. No new permission key (reuses `employeeCompensation.read` following the `payroll` precedent), no schema change, no migration, no new dependency, and no existing report modified — table tooling is opt-in per report and pinned by a regression test. Backend 3257 · Frontend 4030 · Electron 499 tests green with zero new regressions (+27/+6/0, exactly the two new test files); `tsc --noEmit` ×3, `prisma validate` and `migrate status` 64/64 clean, all re-run on `production` post-merge with the merged tree byte-identical to the reviewed branch tree. Packaging audited across `app.asar` (679 entries), `win-unpacked` (3,350 files) and all 4,072 `Setup.exe` entries with zero findings and exactly one `.db` shipped. Golden Database refreshed to the current approved development database and verified byte-identical in four places including extraction from inside the installer, `integrity_check = ok`, 0 foreign-key violations, all 75 schema models present. 9 files (6 modified, 3 new). Full detail: `PROJECT_STATE.md`'s release ledger.) | High |
| **Current stable tag** | `stable-production-release-2026.5.4` (merge commit `b875f9eb`) · Previous `stable-production-release-2026.5.3` (merge commit `b109cfae`) | High |
...
### Latest Release — `stable-employee-compensation-batch-printing-v1` (`876f0ff8`, 2026-08-19)
