...
> Last refreshed: 2026-08-14 (previously 2026-08-11, 2026-08-10, 2026-08-10, 2026-08-10, 2026-08-09, 2026-08-08, 2026-08-08, 2026-08-08, 2026-08-07, 2026-08-07, 2026-08-07, 2026-08-06, 2026-08-06, 2026-08-05, 2026-08-05, 2026-08-04, 2026-08-04, 2026-08-04, 2026-08-04, 2026-08-04, 2026-08-03, 2026-08-03, 2026-08-02, 2026-08-02, 2026-08-01, 2026-08-01, 2026-08-01, 2026-08-01, 2026-08-01, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-31, 2026-07-30, 2026-07-30, 2026-07-30, 2026-07-25, 2026-07-25, 2026-07-24, 2026-07-24, 2026-07-23, 2026-07-23, 2026-07-23, 2026-07-20, 2026-07-20, 2026-07-20, 2026-07-20, 2026-07-20, 2026-07-19, 2026-07-17, 2026-07-01) · This refresh is a release-tracking update, not a read-only audit — it accompanies Production Installer Release 2026.3.2: packages a new self-contained Windows installer with everything merged onto `production` since the 2026.3.1 installer — Production Resource Integrity Audit & Fix Pack v1 (already merged, tagged, documented) — into `AlManarERP-Setup-2026.3.2.exe` (131.76 MiB, SHA-256 `b4ad8c73cb1aa81251f5e68292f890fb9323ca068a987ec0af6bc0b30ea91476`; `win-unpacked` 446.4 MiB). `package.json`'s `version` (`2026.3.1` → `2026.3.2`) is the only tracked-file change. A full release-scope audit against `stable-production-release-2026.3.1..HEAD` confirmed exactly one completed body of work; every other local branch remained excluded work-in-progress. A packaging audit inspected `app.asar` directly (all required files present, 56 migrations, zero real test/debug traces) and flagged one pre-existing, unrelated orphaned asset (`frontend/public/contract_emblem.png`, 15.7 KB, superseded but never deleted by the fix pack) as an out-of-scope housekeeping item. A runtime audit launched the packaged backend standalone against the real seeded database: health check OK, admin login succeeded with the full permission set, an authenticated endpoint responded correctly, all 56 migrations applied cleanly. TypeScript zero errors on all three surfaces; `build:back`/`build:front`/`electron:build`/`npm run dist` all pass. Product Owner manual visual review completed. The previous refresh accompanied Production Resource Integrity Audit & Fix Pack v1: fixes the Employment Contract's Kuwait Public Authority for Manpower emblem not rendering in the packaged production build. Root cause: `EmploymentContractTemplate.tsx` loaded it via a hardcoded root-absolute path (`src="/contract_emblem.png"`, `frontend/public/`) instead of a bundled ES module import — resolves fine under Vite's dev-server origin, but breaks under the packaged app's `file://` load (`base: './'`) since a root-absolute path resolves against the filesystem root, not `frontend/dist/`. Fixed by moving the asset into `frontend/src/assets/` and importing it as an ES module, matching every other bundled image. Audited every other resource-loading path in the project for the same bug class (backend font/DB/attachments/backup resolution, print/PDF `<base href>` composition, branding/signature/stamp data-URL assets, QR/barcode generation, `electron-builder.yml` packaging config) — no other instance found. 2 files (1 modified, 1 new); frontend-only; no schema/permission/route change. TypeScript zero errors on backend/frontend/electron; frontend build verified; 105 targeted frontend tests + full backend suite (186 files/2874 tests) green. Product Owner manual visual review completed. The previous refresh accompanied Production Installer Release 2026.3.1: packages View Zoom Manual Save Pack v1 — the only completed work merged onto `production` since the 2026.3.0 installer, per a full Git audit against `stable-production-release-2026.3.0..HEAD` — into a new self-contained Windows installer (`AlManarERP-Setup-2026.3.1.exe`, 131.75 MiB, SHA-256 `a7e197ad7415a3687b6bc6dd78699b54b78a87969fd90afce886079672807483`; `win-unpacked` 446.4 MiB). `package.json`'s `version` (`2026.3.0` → `2026.3.1`) is the only additional tracked-file change. A packaging audit inspected `app.asar` directly (all expected files present, zero instrumentation traces, zero real test/debug files — only a false-positive filename substring and pre-existing third-party `node_modules` internals). A runtime audit launched the packaged backend standalone against the real seeded database: health check OK, login succeeded with the full permission set, the cheque-designer-templates endpoint responded correctly, all 56 migrations applied cleanly. GUI-only surfaces were not independently re-driven (no interactive desktop session available to Claude) — covered by the Product Owner's completed visual review and by their source being unchanged beyond the already-reviewed zoom pack. The previous refresh accompanied View Zoom Manual Save Pack v1: replaces View Zoom Persistence Pack v1's auto-save design (a `zoom-changed` listener + 400ms debounce) with an explicit manual save, after a live runtime investigation (temporary file-based logging instrumentation on navigation/zoom/focus/blur events, built into a real production installer, run by the Product Owner, then fully reverted with zero trace) found that `zoom-changed` fires only for mouse-wheel zoom — never for the `role: 'zoomIn'`/`'zoomOut'` items the View menu actually uses — so the auto-save path had never captured a menu-driven zoom change, and Chromium's own per-origin resync on the next in-app navigation reapplied the stale disk value, which looked like the zoom reverting on its own. `viewZoomPreference.pure.ts` lost the debounce controller entirely; `main.ts` keeps the same one-time restore-on-launch and adds one View-menu item ("💾 حفظ مستوى التكبير الحالي كافتراضي") that reads the live zoom and writes it synchronously, with no listener of any kind. 3 files (2 modified, 1 test file rewritten: 13 obsolete tests removed, 10 added); electron only; no schema/permission/route change. Product Owner manual visual review completed. The previous refresh accompanied Production Installer Release 2026.3.0: a pure packaging release, no feature branch and no application source/schema/permission change, that repackages every commit already merged onto `production` since the 2026.2.0 installer into a new self-contained Windows installer (`AlManarERP-Setup-2026.3.0.exe`, 131.74 MiB, SHA-256 `0f4e83bce06b281e3ac15287158d6f378bad8ba14d5e6e637632b448ce7a11fd`). `package.json`'s `version` (`2026.2.0` → `2026.3.0`) is the only tracked-file change. Two build-environment defects — both confined to gitignored `node_modules`/`backend/dist`, neither producing a commit — were found and fixed during the build: a stale root-level generated Prisma Client (last regenerated 2026-08-03, predating the 2026-08-08 migrations for `ChequeDesignerTemplate` and `Attachment.content`) that `scripts/prepare-backend-deps.js` would have packaged into the installer's runtime, shipping those two features broken despite a clean compile; and 16 files of orphaned dead code in `backend/dist` (a fully unwired `googleDriveBackup` module, dead entitlement calculators, stray tests) with no `backend/src/*.ts` counterpart, silently entering `extraResources` in prior builds. Both fixed and re-verified directly against the packaged output before shipping. The previous refresh accompanied the User Data Persistence & Legacy Recovery Pack v1 release (internal code name Zero Data Loss Certification Pack v1, used in the branch name, in-code comments, and test file headers): closes three data-loss gaps found in a full-project data-persistence audit covering 19+ storage surfaces. Attachment bytes move from filesystem-only into a new nullable `Attachment.content` column inside `manar.db` — previously invisible to local backup, restore, and Google Drive sync entirely, and silently orphaned by any move to a new machine; `attachments.service.ts` now derives the on-disk path from the CURRENT `ATTACHMENTS_DIR` at read time rather than trusting the absolute path stored at upload time, and self-heals a missing file transparently from its database bytes; `attachments.backfill.ts` migrates every pre-existing attachment's bytes from disk into the new column once at backend startup, never throwing, a no-op after the first run. User-authored preferences — import column-mapping profiles, per-form print-profile/copy-count memory, report and letter favourites/recents, learned Kuwait location usage, sidebar visibility, and the Prices agreements-board toggle — move off `localStorage`-only storage onto the EXISTING `Setting` table (`pref.<userId>.<key>`) via two new routes, `GET/PUT /api/settings/preferences`, scoped server-side to the caller's own key prefix and deliberately requiring no new permission key; reads stay synchronous from the local cache (zero behavior change), writes fan out to the database in a debounced batch, hydrated on login (remote wins) and flushed before logout invalidates the session token. `BackupService.resolveBackupFile()` derives a backup's file path from the current `BACKUP_DIR` with a fallback to the stored path, so restore/verify/remove/pruneAutoBackups keep working after a `productName`/`appId` change relocates `userData` — a proven failure mode, per the Cheque Template Persistence & Legacy Recovery Pack v1 entry below. The audit documented deliberate, unchanged exceptions for genuinely per-device state: the JWT session token, theme/language, table page/search/filter UI state, print-preview feature flags, view zoom, and the OAuth/device-identity files. 27 files (19 modified, 8 new, including `frontend/src/lib/syncedPreferences.ts` and six new regression-test files); backend + frontend; one additive Prisma migration; no new permission key. One pre-existing test (`engineBoundary.test.ts`, asserting the Letter Engine imports nothing outside itself) was preserved rather than weakened by introducing a writer-injection seam (`setFavouritesPersistence`) wired from `main.tsx`; one contract test was updated to match the strengthened backup-restore guarantee (assertion tightened, not loosened). Backend 186/186 files, 2874/2874 tests (+34 new). Electron 24/24, 473/473 (unaffected, unchanged). Frontend 199/207 files, 3757/3783 tests (+18 new) — the pre-existing 26-test/8-file baseline (documented since 2026-07-23) unchanged in count and identity, confirmed by running the full suite both before and after the merge. TypeScript zero errors on all three surfaces; all three builds green, confirmed pre-merge and post-merge. The previous refresh accompanied the View Zoom Persistence Pack v1 release: persists the last zoom level chosen from the View menu's تكبير/تصغير items (Electron's built-in `zoomIn`/`zoomOut` roles, unchanged) and restores it automatically on the next launch, instead of always reopening at Chromium's process-memory-only default of 0 (100%). `viewZoomPreference.pure.ts` (new, no `electron` import) reads/writes a small `view-zoom.json` file under the app's data directory, following the SAME local-JSON-under-dataDir convention already used for other main-process state, written atomically via the existing `writeFileAtomicSync`. The saved level is clamped to Chromium's own safe zoom range (±8 steps, ~25%–500%); writes are debounced (400ms) and flushed on the existing `before-quit` handler. Verified with a real manual visual review on a live `npm run dev` Electron window by the Product Owner — zoom in/out, full close, relaunch, confirmed the same level restored automatically with no side effect elsewhere ("اكتمل الفحص كل شي طبيعي وممتاز"). 3 files (1 modified, 2 new); electron only; no schema/permission/route change. The previous refresh accompanied the Cheque Template Persistence & Legacy Recovery Pack v1 release: closes a confirmed architectural defect in which Cheque Designer templates (the layouts behind the Cheque Template Manager / "Cheque Studio" designer, including a template a real user actually lost) were the only user-created data in the system stored outside SQLite, in browser `localStorage` under `chequeDesigner.templates.v1` — absent from local backup, restore and Google Drive sync, and destroyed outright by a `productName`/`userData` rename or a page-origin change (Chromium partitions `localStorage` by both). Sub-pack A moves the templates into a new `cheque_designer_templates` table (Zod-validated JSON layout with `.passthrough()` so unknown future properties survive), with a one-time, marker-guarded migration off the current browser profile's `localStorage`. Sub-pack B closes the remaining gap with a hand-written, dependency-free LevelDB reader that recovers templates stranded in a PREVIOUS `userData` folder (three legacy folder names taken from the project's own Git history), strictly read-only, gated on four server-checked preconditions and its own independent marker — verified against real data on the reporting machine, recovering all 6 stored templates including the correct default. 24 files (15 new, 9 modified); backend + frontend + electron; no new permission key; one new table. The previous refresh accompanied the Form Editor UX Rebuild Pack v2 release: rebuilds the Official Letter page into a lightweight, generic Form Editor, on top of Form Editor UX Simplification Pack v1 carried on the same branch. Version History and Comments removed completely (backend + frontend); validation rules cut from 25 to exactly 3 (E4_reservedZoneOverlap, E16_objectInReservedZone, E13_impossibleGeometry — deleted, not deselected); date/recipient/subject stopped being fixed sections rendered on the sheet, kept only as metadata (barcode payload, registration snapshot, workspace columns/search/sort unaffected); real Word (.docx) export via the `docx` package, mapping the Block Model directly rather than the rendered page, with a native save dialog through a new Electron IPC channel; accurate preview wired to the shared `useAccurateFormPreview` infrastructure 16+ other forms already use; `ExecutiveHeader`'s identity-card styling replaced by a slim merged toolbar shell; Insert promoted to a primary, default-open left rail; the Object Inspector/Document Properties rail moved to a contextual right-hand slot. 64 files (46 modified, 12 deleted, 6 added); backend + frontend + Electron; no Prisma/schema/permission-key change. The previous refresh accompanied the Financial Position Analysis Audit & PDF Fix Pack v1 release: a PDF export fix and a full calculation audit for the Financial Analysis Center. `composeStyledFromNode` cloned only the print root for export, losing its `.xpl-scope`/`.xpl-page`/`.fac-page` ancestors and every rule/token scoped to them — including the table-wrap override that keeps `@media print` from clipping every table at the shared kit's `max-height: 62vh` — fixed by composing a detached clone wrapped in that same ancestor chain; KPI cards separately could clip an amount with no ellipsis because `useFitText`'s inline font-size survives unchanged into the static document, fixed with new print-only rules. The calculation audit corrected three engine defects: a period day-count off-by-one inflating DSO and mis-sizing the previous-period comparison window; Section 5 Receivables computed as a period-movement delta instead of an as-of balance (via a new `AnalysisDataset.ledger`, zero added queries, matching the project's own AR definition); and Section 8 DSO reading Section 4's signed movement delta instead of Section 5's real balance. A follow-up correction, made after an explicit review question about Section 4's collection-rate definition, replaced a formula mixing collected-in-period against invoiced-in-period only (producing rates over 900%) with the Collection Effectiveness Index against opening AR plus period sales, and moved the section's default row sort to total collectible accordingly. 11 files modified, backend + frontend; no Prisma/schema/migration/permission-key/Electron change, no page redesign. The previous refresh accompanied the Document Studio UX Polish Pack v1 release: UX/UI polish for the Official Letter page's Document Studio, no new business feature and no document-model/print-engine/pagination/backend change. A single documented z-index ladder plus a portal-based `useFloatingPosition` hook fix the reported "Font dropdown hidden behind other panels" at its real cause — `DocumentToolbar`'s own `overflow-x: auto` clipping the dropdown before z-index was ever consulted — applied to FontPicker's dropdown, the toolbar's Spacing popover, and a new floating selection toolbar that appears on an actual text selection and reuses the docked toolbar's own commands. Resizable side rails, collapsible Object Inspector cards, and Layers-panel/canvas interaction polish. A live-browser pass against the running dev server caught and fixed two real defects static review missed: the new resize handles on two panels were unclickable, silently clipped by that panel's own `overflow-y: auto`; and every handle had a dead zone under its panel's sticky header from an insufficient z-index. 33 files (6 new, 27 modified), frontend-only. The previous refresh accompanied the Document Studio v1 release: the Official Letter page, transformed from a basic rich-text editor into a professional Document Studio across three additive packs merged as one release — Foundation v1 (canvas, rich text, productivity, navigation), Layout Designer v1 (a free object layer over the flow document, layers panel, smart guides — the letterhead's reserved bands stay a blocking rule regardless of object placement), and Professional Document Automation v1 (an 18-variable engine resolved live on a DRAFT and frozen elsewhere, asset/block/template libraries, version history with two never-pruned lifecycle snapshot kinds producible only by the server itself, a word-level track-changes diff engine, threaded comments, and Smart Export through the existing print pipeline). Two new tables (`LetterVersion`, `LetterComment`); no new permission key. 100 files (66 new, 30 modified, 4 deleted), backend + frontend; every file staged by explicit path, leaving a large unrelated pre-existing Electron packaging/startup-window workstream sharing the same working tree untouched. The refresh before that accompanied the Collection Analysis Page v1 release: a hidden analytical page answering how invoices were collected across fiscal years, built on a dedicated `CollectionAnalysisEngine` that imports its business rules from `shared/services/operational.reporting` rather than re-implementing them, with five professional tables including a data-derived transition matrix, plus a shared KPI-card overflow fix reaching all 24 `MetricCard` consumers. 38 files (27 new, 11 modified); no Prisma/schema/migration, no new permission key. The refresh before that accompanied the Administrative Forms Barcode Enhancement Pack v1 release: a barcode as a THIRD element of the existing Multi-Signature & Stamp branding system on the Blank A4 administrative form (`ElementType` extended to `'signature' | 'stamp' | 'barcode'`, carried through the same designer hook, image component, gestures, save path and print/PDF/preview pipeline every element already used — no new engine, store, service, or renderer), a Barcode Content Settings dialog (reference number, subject, free-text details — built from existing ExplorerKit primitives) that extends `FormQRCode`'s text-building logic additively so the thirteen pre-existing QR consumers encode byte-identical text, removal of the previous clock-derived caption number in favor of the operator's own reference (or no caption at all), a pure width-preserving reference-number-increment suggestion plus a fourth settings key remembering the last non-empty reference independently of a Reset click, and a Professional Ink Set v1 that appends 20 ballpoint-blue shades after the 4 pre-existing ones so a saved color choice never resolves differently. 21 files (2 new, 19 modified), frontend-only; no Prisma/schema/backend/Electron change, no new permission key. The refresh before that accompanied the Cloud Backup & Google Drive Sync v1 release: four packs delivered as one feature. The Google Drive Deployment Pack bundles the OAuth Desktop client into the installer and resolves it through a pure, unit-tested policy layer, so the end user never handles a credentials file. The Cloud-Failure Local Backup Guarantee makes every failed cloud operation leave a local backup, with a direct `VACUUM INTO` path for the shutdown-sync case where the backend is already stopped. The Production Hardening Pack closes the audit's three blocking defects — an invisible and unrecoverable dead OAuth grant (now a distinct `GRANT_DEAD` state that deletes the dead token, persists the reason across restarts and offers one-click recovery), silent cross-device overwrite (now optimistic-concurrency checked and turned into a user-resolved conflict) and the total absence of network timeouts (now real `AbortController` deadlines on every Drive call) — and adds OAuth client binding on stored tokens, one central Arabic error-translation layer, a sync mutex, atomic state writes, 403 rate-limit handling with `Retry-After`, and full test coverage of the previously untested decision engine. The Diagnostics and Polish packs turn the backup page into a cloud diagnostics centre: sixteen status items, a 0-100 health score with history, an operation log with durations and a details drawer, one-click self repair, engine information, and PDF/Excel diagnostics exports — all built from existing ExplorerKit primitives with zero new CSS, and all read passively with zero network calls. 33 files, Electron main process + backend + frontend; no Prisma/schema/migration and no new permission key. The refresh before that accompanied the Financial Analysis Center v1 release: a new read-only executive page at `#/financial-analysis` (guarded by `reports.read`) with eight table-only sections — profitability, revenue, expenses, collections, receivables, monthly performance, top lists and financial indicators — built on a new `financialAnalysis` module split into a shared type contract, a single Prisma dataset layer, a pure compute layer and thin service/controller/routes. Business rules are not redefined: revenue, expenses, collections and P&L come from `shared/services/operational.reporting`, so the page matches the dashboard and the P&L report by definition. One dataset load per request feeds all eight sections; every section KPI is derived from its own table rows. Ships an 11-worksheet Excel export (shared `buildExcelWorkbook`) and a landscape multi-page PDF export (shared `composeStyledFromNode` + Electron `exportPdfFromHtml`), plus a root-cause fix to `useT()`'s unstable identity that had produced an unbounded render/fetch loop. 32 files, backend + frontend; no Prisma/schema/migration/permission-key/Electron change. The previous refresh accompanied the Employee Table Column Optimization Pack v1 release: narrows the nationality column and 8 neighboring Employees-table columns so the license-expiry column sits ~209px closer to the visible viewport at the project's standard 1440×900 window, without reordering columns or touching sort/filter/search/data logic. 1 file, frontend-only; no Prisma/schema/backend change.
...
### Latest Release — `stable-view-zoom-persistence-pack-v1` (`e0467d62`, 2026-08-08)
