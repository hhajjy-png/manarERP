...
| **Current HEAD** | `ec874c8b` — direct commit to `production`, no feature branch this cycle (documentation commit to follow) | High |
| **Current stable tag** | `stable-production-release-2026.3.0` (hash-closure commit) | High |
| **Previous stable tag** | `stable-user-data-persistence-legacy-recovery-pack-v1` (`39da6141`) | High |
| **Application version** | `2026.3.0` — calendar versioning; installer `AlManarERP-Setup-2026.3.0.exe` (131.74 MiB, Windows 10/11 x64, per-user install under `%AppData%`) | High |
