...
| **Current HEAD** | `d94dfe26` — merge of `feature/production-resource-integrity-audit-fix-pack-v1` (documentation commit to follow) | High |
| **Current stable tag** | `stable-production-resource-integrity-audit-fix-pack-v1` (merge commit `d94dfe26`) | High |
| **Previous stable tag** | `stable-production-release-2026.3.1` (`0712c905`) | High |
| **Application version** | `2026.3.1` — unchanged, source-only fix/audit pack; last installer `AlManarERP-Setup-2026.3.1.exe` (131.75 MiB, Windows 10/11 x64, per-user install under `%AppData%`) | High |
