...
| **Current HEAD** | `66f03cf7` — merge of `feature/xbrl-readiness-foundation-v1` (**XBRL Readiness Foundation v1**), on top of `bd15959f` (**Migration History Reconciliation v1**). A standalone preparation layer above the existing accounting system that makes manarERP **XBRL-ready**. **Not QAYD-certified**: no official Kuwaiti/QAYD taxonomy is installed, no government API is assumed, no official XBRL file is produced, and no compliance with QAYD or the Ministry of Commerce is claimed — the highest displayable state is `READY_PENDING_TAXONOMY` and there is no `QAYD_READY` state. Purely additive migration `20260819120000` (six new `xbrl_*` tables, zero `ALTER` on any existing table, zero data rows read or modified); three new permission keys; zero effect on GL, posting, payroll, employee compensation, cheques or inventory — enforced by a static test over the whole module source. Source-only release; `package.json` stays at `2026.5.4`. Validation on `production` after the merge: `prisma validate` ✅, `migrate status` 69/69 "Database schema is up to date" ✅, migration drift zero in both directions ✅, backend/frontend `tsc --noEmit` ✅, targeted XBRL + migration + accounting suites 12 files / 153 tests ✅, frontend XBRL contract test 18 ✅. Product Owner manual visual review completed and approved. Full detail: `PROJECT_STATE.md`'s release ledger. | High |
| **Current stable tag** | `stable-xbrl-readiness-foundation-v1` (merge commit `66f03cf7`) · Checkpoint `checkpoint-xbrl-readiness-foundation-v1` → `bd15959f` (production HEAD immediately before the XBRL merge) · Previous `stable-employee-compensation-batch-printing-v1` (merge commit `876f0ff8`) | High |
...
### Latest Release — `stable-xbrl-readiness-foundation-v1` (`66f03cf7`, 2026-08-19)
