...
| **Current HEAD** | `581ad7ee` — merge of `feature/vehicle-insurance-recovery-v1` (**Vehicle Insurance Management v1**). A self-contained operational module for equipment insurance policies and accident history, living in two new tables with a foreign key to `equipment` and nothing else: no journal entries, no expenses, no maintenance, no claims, and `equipment.insuranceExpiry` (read by the Document Expiration Center) untouched. Renewal creates a new policy rather than mutating the old one and accidents are a permanent log, so the module exposes no delete path and no `delete` permission key. This is a **recovery, not a re-implementation**: the package was written in `353e71d0` and never reached production because a `rebase --onto production` dropped it during the batch-printing release; it was restored from the original branch as the sole source, with the twelve package-owned files byte-identical to it. No new migration — `20260818120000_add_vehicle_insurance` was already applied and already on production, and both Prisma models were already declared structurally, so nothing was duplicated. Source-only release; `package.json` stays at `2026.5.4`. Validation after the merge: `prisma generate`/`validate`/`migrate status` 69/69 ✅, backend/frontend/electron `tsc` ✅, both builds ✅, backend 3418 · frontend 4088 · electron 488 tests ✅, zero regressions. Product Owner manual visual and functional review completed and approved. Full detail: `PROJECT_STATE.md`'s release ledger. | High |
| **Current stable tag** | `stable-vehicle-insurance-management-v1` (merge commit `581ad7ee`) · Checkpoint `checkpoint/pre-vehicle-insurance-v1` → `13f5ee47` (production HEAD immediately before the merge) · Previous `stable-xbrl-readiness-foundation-v1` (merge commit `66f03cf7`) | High |
| **Previous stable tag** | `stable-production-release-2026.5.3` (merge commit `b109cfae`) — the previous installer release | High |
| **Application version** | `2026.5.4` — new installer build; installer `AlManarERP-Setup-2026.5.4.exe` (131.93 MiB, 138,339,925 bytes, SHA-256 `3f79dae177b809d92733f8015f797ba0d98a6314f0e8e92c65152256edbe1aca`, Windows 10/11 x64, per-user install, zero external runtime prerequisites); `release/win-unpacked` 3,350 files / 469,399,374 bytes. Golden Database SHA-256 `8ecaa3988d8171c6f861468177943fedb9be3427c3f4ef51a219cc4a0f51b585` (3,686,400 bytes · `integrity_check = ok` · 0 foreign-key violations · all 75 schema models present · 64/64 migrations) verified byte-identical in four places including extraction from inside `Setup.exe`. Ships the same 64 migrations and 75 models as 2026.5.3 — no schema change | High |
