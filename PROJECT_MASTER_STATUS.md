...
> table below at the current HEAD, consistent with every other table-only pass in this history. **Rotation status — now closed:** `PROJECT_STATE.md` was rotated on
> **2026-09-08** (*PROJECT_STATE Documentation Rotation & Maintenance v1*): 172 older release entries were
> moved verbatim into `docs/history/`, leaving the current release plus the 15 most recent in the live file.
> The maintenance item flagged in this and earlier passes — a release log grown far past its own ~15-entry
> trigger (185 `## Previous Release` sections by the time it was rotated) — is therefore **resolved**.
...
| **Current HEAD** | merge **`814116b4`** — **Employee Debt Acknowledgment Administrative Form v1** (feature release), parents `a3c5b18e` (production) and `618f00b0` (feature), tag `stable-employee-debt-acknowledgment-form-v1` → that merge commit; release documentation in `291e1a20` with a hash-closure commit on top. Released 2026-09-10, Product Owner Visual Review **APPROVED**. **This is the first code-bearing release since Production Release 2026.5.9** (`ab9480d9`) — the three releases between them were documentation only. It ships an administrative form in Arabic/English/Hindi with its own letterhead print profile; **no schema change, no migration, no new permission key, no new dependency**, and `package.json` is untouched, so the Desktop installer release stays **2026.5.9** and no installer was built. `production == origin/production`. Previous HEAD: `f37c9c7f` — **Complete User Manual v1** (documentation release), merge `57e2c8a1`, tag `stable-complete-user-manual-v1`. **Note:** the HEAD chain recorded below was last repointed at `ac05c45c` and does not list every release merged between then and now — the authoritative release log is `PROJECT_STATE.md` (current + 15 most recent) and `docs/history/` (everything older). Previous HEAD: `ac05c45c` — merge of `feature/cheque-calibration-profiles-v1` (**Simple Cheque Calibration Profiles + Persistent Default v1**, feature commit `9db0051f`). Adds three fixed calibration profiles per bank cheque template — `office` (المكتب), `home` (البيت), `other` (أخرى). The identity of a calibration becomes the pair `bank template + calibration profile`: nine independent `settings` rows for three banks, keyed `cheque.calibration.<bank>-a4.<profile>.v1`, each read and written on its own key, so calibrating Gulf at the office cannot move Gulf at home and neither can touch KFH or NBK. A pair never saved before starts from **its own bank's factory document** — never from another profile's edits and never from another bank. **Legacy compatibility:** a calibration saved before profiles existed lives on the template's un-suffixed key and is inherited by **`home`**, the printer it was actually made on, so the Gulf calibration already in production keeps printing exactly as it did; `office` starts from Gulf factory geometry until calibrated in its own right. Inheritance is strictly a fallback — the pair's own row is looked up first and always wins, whatever the row order. The legacy row is read, never written, never deleted; no migration. **Persistent default:** the chosen profile is saved to one app-wide row, `cheques.defaultCalibrationProfile`, holding one word; the last choice survives closing and reopening the app until the user changes it, and `office` is the first-run value and the fallback for a missing or invalid row, nothing more. The cheques page owns that state and hands it to the Professional Calibration Studio, which is now **controlled** — so page and studio can never disagree, and preview and physical print resolve the **same document**. A batch print mixing bank templates is now refused outright rather than printing every cheque with the first one's geometry. No geometry, field position, offset, print engine, calibration engine, renderer, DB model, API, migration or permission key changed; no `localStorage`. `GULF_BANK` APPROVED · `KFH` and `NBK` PROVISIONAL are untouched, and their production printing stays blocked under every profile. 55 new targeted tests across two suites; frontend typecheck and build clean; full frontend suite 224 files / 4141 tests green; no installer rebuilt. Product Owner visual review completed and approved before merge. Two cheque releases merged between this entry and the previous HEAD recorded below and did not update this table at the time: `2ea4aae8` (**Gulf Bank A4 Cheque Template + Professional Calibration + Date Block v1**) and `0a015f95` (**Multi-Bank Cheque Profiles Readiness v1**); both are recorded in `PROJECT_STATE.md`. Previous HEAD: `ac7cb93d` — merge of `fix/financial-accuracy-unified-v1` (**Financial Accuracy & KPI Integrity v1**, pack commits `e693b1e3` / `b9055f7a` / `d5cb128e` / `71b48b89`). Four packs released together, correcting KPI values that were provably wrong against the database, then removing the ambiguity that produced them. **Pack 1** fixes three confirmed wrong values: the "Critical +90 days" cards read `0.000` KWD while `24,586.000` was genuinely aged (the filter used `dueDate` alone, and every invoice carries `dueDate = NULL`, which Prisma's `lt` never matches — it now falls back to `issueDate`, matching the receivables-ageing report); bank reconciliation workspace totals showed whole-file figures above a row list holding only inserted rows (import #36: `81,939.400` vs `1,500.100`, now separate fields); and the monthly invoice report collapsed 70 of 113 invoices — `680,868.000` KWD, 65% of value — into one unlabelled period row because `billingYear` is NULL on historically imported invoices, now falling back to `issueDate` for display only. Also: reconciliation search and amount filters combined as OR instead of AND; employee search filtered `'active'` against the stored `'ACTIVE'`, returning nothing; two of three report cards without a backend implementation gained one. **Pack 2** is labels and descriptions only — no formula changed: "collection rate" carried four distinct formulas and is now named Collection Effectiveness (CEI) / cumulative / in-scope / period; "total expenses" was one key for both the approved-only operational metric and the all-statuses page total; the revenue donut summed only the top five customers under the caption "Total revenue"; Financial Analysis §4 now renders the opening receivables balance already present in its payload so the closing-balance equation is legible. `docs/financial-kpi-definitions.md` records the contract. **Pack 3** aligns card and table filters (expenses search reaching the cards, payroll "paid" no longer overriding the user's status filter), moves statements and maintenance from UTC midnight to the project's local-day boundary — which in Kuwait had been dropping the first three hours of the start day and most of the end day — clamps partial profit-and-loss ranges that previously expanded to whole months, excludes cancelled maintenance from due work, and stops failed requests from rendering `0.000` KWD or retaining the previous filter's numbers. **Pack 4** unifies five parallel frontend rounding families into `lib/money` mirroring the backend policy, routes the bank statement module and statement opening balances through `roundMoney`, applies write-time normalisation to expense amounts, and replaces three different day-counting implementations with one contract where today stays `0` for the whole local day in Kuwait. Repeated thresholds move to `config/thresholds` with purpose-describing names and identical values — the two high-debt thresholds stay separate at 5,000 and 10,000 because they serve different cards. User counts now read `meta.total` and three dropdowns use `fetchAllRows` instead of silently truncating at 200. A permanent cross-surface parity contract covers receivables, revenue, expenses, collections, +90 ageing, monthly invoices and reconciliation totals. **73 files, all attributable to Packs 1–4**; unrelated pre-existing work that had been mixed into the development branch was deliberately excluded and remains preserved on `fix/financial-accuracy-hotfix-v1`. No schema change, no migration, no new permission key. Deliberately unchanged pending product decisions: `OVERDUE` derivation, payroll bank export `APPROVED`/`PAID`, and `POTENTIAL_DUPLICATE` in bank balances. Backend 3613 · frontend 4157 green; `package.json` unchanged — backend/frontend pack, no installer rebuild. Product Owner visual review completed and approved before merge. Previous HEAD: `a684c66a` — merge of `feature/multi-bank-cheques-foundation-v1` (**Multi-Bank Cheques Foundation v1**, feature commit `c2de9a12`). Turns bank identity in the cheques module from a free-text `Cheque.bankName` string into a real `Bank → BankAccount → Cheque` structure. Two new tables (`banks`, `bank_accounts`) plus `cheques.bankAccountId`; one migration `20260821120000_multi_bank_cheques_foundation_v1` — **70 migration folders**, `migrate status` up to date, zero drift. Cheque-number uniqueness moves from global to **per bank account** (`@@unique([bankAccountId, chequeNumber])`), with a service-level probe covering the `NULL` scope SQLite unique indexes do not guard. **Cheque numbers remain manually entered** from the physical pre-printed cheque — no automatic numbering, no next-number suggestion, no `ChequeBook`. Backfill seeds the ten Kuwaiti banks under stable non-Arabic `code` identifiers, creates Gulf Bank's «الحساب الرئيسي» — the only account carrying an approved print profile — and links its legacy cheques by an **exact, narrow** `bankName` match: 56/56 linked, zero unlinked, and not one cheque number, amount, date, status or print-log row altered. A legacy cheque with no account is **not** printable merely because the account is missing: it prints only if its bank name matches a bank that really has a print-enabled account, so an unknown legacy bank is never guessed and never printed with another bank's template. **Gulf Bank Classic printing is unchanged** — same provider, dimensions, coordinates, calibration, preview, print and batch print. Printed output stays ink-only (date, payee, numeric amount, tafqeet); no cheque image, bank logo or background ever prints, proven on both the Classic and Designer paths. New banks are record-ready but **not print-enabled** until real cheque dimensions and an approved profile are supplied, guarded independently in five places with no fallback to Gulf Bank's image, sizes or coordinates. Bank-statement matching becomes account-aware and returns an explicit `ambiguousChequeNumbers` warning instead of arbitrarily picking one of two cheques sharing a number. New `/banks` management page, new `banks.read`/`banks.manage` keys synchronised **inside the migration itself** (the automatic `prisma migrate deploy` path) rather than by a manual reseed. No GL integration, no `ChequeBook`, no multi-bank calibration or designer, and no default templates or dimensions for new banks. 41 files, +3,410/−129; 41 new tests; backend 3512 · frontend 4109 green; `package.json` unchanged — backend/frontend pack, no installer rebuild. Product Owner visual review completed and approved before merge. Previous HEAD: `0d33b9ea` — merge of `feature/production-release-2026.5.6` (**Production Release 2026.5.6**). Packaging release: `package.json` `2026.5.5` → `2026.5.6` is the only tracked-file change in the release commit. Bundles into a new self-contained Windows installer everything merged onto `production` since the 2026.5.5 installer — **C-1 Critical Accounting Fix Pack v1** (`ffa03704`, merge `1963201a`) and **Packaging Hardening Pack v1** (`3feb15e3`, merge `788c8fe0`). No schema change, no migration, no new permission key, no UI change. Two long-standing packaging defects are closed at the root in this release: the repo-root Prisma client drift (75 models vs the schema's 85 in 2026.5.5) is fixed by sourcing the client from `backend/node_modules` plus a fail-closed model-set guard that ran in this build and confirmed 85/85; and `__livetest__/liveTest.js`, verifiably shipped inside the 2026.5.5 installer, is now excluded by two independent barriers and confirmed absent from all 4,107 `Setup.exe` entries. Installer `AlManarERP-Setup-2026.5.6.exe`, SHA-256 `2b44d5a677f7f29a78d15f6eac373204d56cc928d967fd5f8285e8388bb6e44e`. Hash-closure documentation commit `2ecf7b26` sits on top. Previous HEAD: `1963201a` — merge of `feature/c1-critical-accounting-fix-pack-v1` (**C-1 Critical Accounting Fix Pack v1**). Source-only correctness release, no installer and no version bump (`package.json` stays `2026.5.5`). Closes one critical accounting defect: `PUT /invoices/:id` allowed changing a purchase invoice's `paymentMethod` after GL posting. The field drives two outputs on different lifecycles — the journal entry's credit account, re-derived on every `repostInvoiceToGL`, and the settlement state (`paidAmount`/`status`), derived from it only once at creation via `isImmediatelySettledPurchase`. Editing it moved the first and froze the second: the ledger credited Cash (settled) while the invoice stayed `UNPAID`, so `addPayment`'s guard — reading a now-false `paidAmount` — passed and credited Cash a second time, leaving Accounts Payable with a debit balance (an accounting-impossible negative liability); in the `PARTIAL` case the corruption was immediate, with no further payment needed. Fix: early-reject guard in `update()` before any DB work, comparing the **effective** credit account (`null` ≡ `ACCOUNTS_PAYABLE`) so accounting-equivalent edits still pass, and scoped to the **final** direction so the `SALES → PURCHASE` + cash-method variant is closed too. Three files, +261/−2; no schema change, no migration, no new permission key, no UI change. Exposure at release: zero purchase invoices and zero purchase journal entries in the database, and no screen or import path sends the field. Backend 3434 tests green (baseline 3418 + the pack's 16). Hash-closure documentation commit `bd60bd92` sits on top. Previous HEAD: `95f67a9c` — merge of `feature/production-release-2026.5.5` (**Production Release 2026.5.5**). Packaging release: `package.json` `2026.5.4` → `2026.5.5` is the only tracked-file change. Bundles into a new self-contained Windows installer every Feature Release merged onto `production` since the 2026.5.4 installer — **Employee Compensation — Batch Printing v1** (`78cbab4b`, merge `876f0ff8`), **Migration History Reconciliation v1** (`99b89c68`, merge `bd15959f`), **XBRL Readiness Foundation v1** (`2d221663`, merge `66f03cf7`), **Vehicle Insurance Management v1** (`b80b8c4b`, merge `581ad7ee`). No schema change, no migration, no new permission key. The recurring root-`node_modules` Prisma client drift (75 models vs the schema’s 85) recurred and was neutralised before packaging; the shipped client carries all 85 models with a model set identical to the schema. Hash-closure documentation commit `ffec75a4` sits on top. | High |
| **Current stable tag** | `stable-employee-debt-acknowledgment-form-v1` (merge commit `814116b4`) · Checkpoint `checkpoint/pre-employee-debt-acknowledgment-v1` → `a3c5b18e` (production HEAD immediately before the merge; created when the package started and reused, not recreated) · Before it: `stable-complete-user-manual-v1` (merge commit `57e2c8a1`) · Checkpoint `checkpoint-complete-user-manual-v1` → `00ef23c8` · Before it: `stable-cheque-calibration-profiles-v1` (merge commit `ac05c45c`) · Checkpoint `checkpoint-cheque-calibration-profiles-v1` → `0211cc23` (production HEAD immediately before the merge) | High |
| **Previous stable tag** | `stable-complete-user-manual-v1` (merge commit `57e2c8a1`) · Before it: `stable-production-release-2026.5.9` (release commit `ab9480d9`) — **still the current Desktop installer release** · Before it: `stable-multi-bank-cheque-profiles-v1` (merge commit `0a015f95`) · Checkpoint `checkpoint-multi-bank-cheque-profiles-v1` → `ce40ad09` · Before it: `stable-gulf-a4-cheque-template-v1` (merge commit `2ea4aae8`) · Checkpoint `checkpoint-gulf-a4-cheque-template-v1` → `60b4ada8` · Before it: `stable-financial-accuracy-kpi-integrity-v1` (merge commit `ac7cb93d`) · Checkpoint `checkpoint-financial-accuracy-kpi-integrity-v1` → `f5b0f14f` · Before it: `stable-multi-bank-cheques-foundation-v1` (merge commit `a684c66a`) · Checkpoint `checkpoint-multi-bank-cheques-foundation-v1` → `c40d14ef` · Before it: `stable-production-release-2026.5.6` (merge commit `0d33b9ea`) — the current installer release · Checkpoint `checkpoint-production-release-2026.5.6` → `788c8fe0` | High |
| **Application version** | **`2026.5.9`** — read from `package.json`. **Production Release 2026.5.9**, release commit `ab9480d9`, tag `stable-production-release-2026.5.9`, released 2026-09-07. Bundles the three packs merged since the 2026.5.8 installer (Leave Request Editable Request Date v1, Payment Voucher Print Fix & Letterhead Template v1, Receipt Voucher Company Letterhead Template v1). Migrations 70 → **71** (`20260907120000_add_leave_request_date`, additive), **87** Prisma models, no new permission key. Installer `AlManarERP-Setup-2026.5.9.exe` (138,517,894 bytes, SHA-256 `30743922f57559ab37127bbe443336225c0f80b37207e6ba84219787ff65069e`). **Both smoke tests passed** (packaged backend and GUI), correcting the environment limitation recorded for 2026.5.8 below — its cause was `ELECTRON_RUN_AS_NODE=1` in the build shell, not a defect. **Unchanged by the two documentation releases that followed**, neither of which touched `package.json` Previous: **`2026.5.8`** — **Production Release 2026.5.8**, built from `production` HEAD `ef1674e4`, release commit `38414d42`, tag `stable-production-release-2026.5.8`. A new self-contained Windows installer bundling the four packs merged since the 2026.5.7 installer (Bank Salary Analytics — Western Digits Consistency v1, Expiration Center — Single Source of Truth & Data Integrity v1, Remove Employee Vehicle License Expiry v1, Employee ↔ Payroll Eligibility & Status Transition Integrity v1); `package.json` is the only tracked-file change in the release commit. No new migration (70), no schema change (87 Prisma models verified identical to the schema by the fail-closed build guard and by extraction from inside `Setup.exe`), no new permission key, no new dependency. Installer `AlManarERP-Setup-2026.5.8.exe` (132.07 MiB, 138,492,514 bytes, SHA-256 `ddeea3c66f7849c9f0a32543add0b17a8beadeed2360be6bba449d59f15b3a10`); `win-unpacked` 3,384 files / 470,406,179 bytes; `app.asar` 688 entries. Golden Database SHA-256 `dda5d4503ee05b286bbb80ed80cfba292e2f5bc8ab31b72f442a330505edb215` (4,075,520 bytes) byte-identical across source, `win-unpacked`, inside `Setup.exe` and its manifest. Packaging audit clean. **Smoke test partial:** the packaged backend passed end to end (port 48211, `/api/health` ok, shipped Golden Database opens with 70 migrations applied, live Prisma query path correct, empty `error.log`), but the packaged **GUI would not start on the build machine** under any launch mode or GPU workaround flag — the previously installed 2026.5.6, which passed a full GUI smoke test at its release, fails identically now, so the cause is environmental rather than a defect in this build. The main window and in-app version number are **not** visually verified and must be confirmed on an end-user machine before distribution. Previous: `2026.5.7` — installer 138,478,346 bytes, SHA-256 `89d4a8c6d10ff705073da2213c8ce43d0538c28848900b695d8d47a463800914`, 70 migrations, 87 models | High |
...
| **Prisma migrations** | **71** — counted in `backend/prisma/migrations/`; latest `20260907120000_add_leave_request_date` (additive). Matches what `PROJECT_STATE.md` records | High |
| **Prisma models** | **87** — counted as `^model ` blocks in `backend/prisma/schema.prisma`. Matches what `PROJECT_STATE.md` records | High |
| **User documentation** | **Complete User Manual v1 — RELEASED** (2026-09-08). `docs/user-manual/build/AlManarERP-User-Manual-2026.5.9-AR.pdf` — 128 pages, 31 chapters, 75 figures, Arabic/RTL, documenting Desktop Production 2026.5.9. Editable source and coverage matrix under `docs/user-manual/` | High |
| **Release-log structure** | `PROJECT_STATE.md` holds the current release + the **10** most recent `## Previous Release` entries. The **180** older entries were rotated out verbatim into `docs/history/PROJECT_STATE_ARCHIVE_*.md` (**5 files**) — 172 on 2026-09-08 and 8 more on 2026-09-10 during this release, when the log reached 19 entries and crossed the ~15 trigger. Index in `docs/history/README.md`, full accounting for the first batch in `docs/history/ROTATION_MANIFEST_V1.md`. **No release history is duplicated into this file** | High |
...
### Latest Release — `stable-vehicle-insurance-management-v1` (`581ad7ee`, 2026-08-19)

**Vehicle Insurance Management v1** — merge of `feature/vehicle-insurance-recovery-v1` (one feature commit `b80b8c4b`, 24 files: 12 new, 12 modified). Source-only release; no installer build and `package.json` stays at `2026.5.4`.

A self-contained operational module for vehicle insurance policies and accident history. It lives in exactly two new tables — `vehicle_insurance_policies` and `vehicle_accidents` — each with a foreign key to `equipment` and nothing else. **No journal entries, no expenses, no maintenance records, no claims**, and `equipment.insuranceExpiry` (the field the Document Expiration Center reads) is untouched: the module neither writes to it nor reads from it, so that screen behaves exactly as before. Renewal creates a **new** policy row instead of mutating the previous one, and accidents are a permanent historical log — so the service exposes no delete path at all, and there is deliberately no `delete` and no `approve` permission key.

**This release is a recovery, not new work.** The package was implemented in `353e71d0` on `feature/vehicle-insurance-v1` and never reached production: the batch-printing branch was created on top of it and inherited its two commits, then was rebased `--onto production` at release time, dropping them from the line of work. That was a deliberate, documented scope narrowing, and the code was never deleted — no delete commit for any of its files exists anywhere in the repository. Recovery used the original branch as the sole source via `merge --squash`; the twelve package-owned files are byte-identical to their originals, and not one line was hand-copied or re-written.

**No new migration.** `20260818120000_add_vehicle_insurance` was already applied to the development database and already present on production since **Migration History Reconciliation v1**; it was not touched. Both Prisma models were likewise already declared there as structural-parity models, so they were not duplicated — they were moved into their own section and the reconciliation block's header was corrected so it no longer describes the module as inactive. The schema stays at 85 models and the migration folder at 69.

**Seven conflicts, all pure time drift**, resolved by keeping both sides: `constants.ts` (2), `app.ts` (2), `seed.ts` (1), `schema.prisma` (3, comments only) and `routerFutureFlags.test.tsx` (1). No production line was replaced by an older version — only two functional lines were removed, both superseded: the `EQUIPMENT_MANAGER` permission list (now including `vehicleInsurance`) and the lazy-route counter. Everything else removed was a comment.

**A pre-existing defect was closed in passing:** `routerFutureFlags.test.tsx` asserted 64 lazy routes while production actually had 65 — a stale assertion recorded in `AI_CONTEXT.md` under pending work. It is now 66, counted from `App.tsx` after the route was added.

**Validation:** `prisma generate` ✅ (clean, after closing the running Electron app that had been locking the Prisma query engine) · `prisma validate` ✅ · `migrate status` 69/69 ✅ · Backend / Frontend / Electron `tsc --noEmit` ✅ · `build:back` ✅ · `build:front` ✅ · Backend 212 files / 3418 tests ✅ · Frontend 223 files / 4088 tests ✅ · Electron 25 files / 488 tests ✅ — all re-run on `production` after the merge, zero regressions. Product Owner manual visual and functional review: **completed and approved**.

### Previous Release — `stable-xbrl-readiness-foundation-v1` (`66f03cf7`, 2026-08-19)

**XBRL Readiness Foundation v1** — merge of `feature/xbrl-readiness-foundation-v1` (one feature commit `2d221663`, 43 files), merged on top of `bd15959f` (**Migration History Reconciliation v1**, commit `99b89c68`, 5 files) so migration history stays consistent. Source-only release; no installer build and `package.json` stays at `2026.5.4`.

A standalone preparation layer above the existing accounting system, so that Kuwait's QAYD taxonomy can be added later without redesigning anything accounting-side. The target path is Accounting → Financial Statements → XBRL Mapping Layer → Validation → Export; this release builds the three middle links and leaves Export as a deliberate barrier.

**Not a compliance claim.** manarERP is **XBRL-ready only, not QAYD-certified**. No official Kuwaiti taxonomy is installed — `isOfficial` is not settable from any API path in v1, so naming a taxonomy "KW-QAYD-2027" does not make it official. No government API is assumed (there is not one HTTP client in the module), no official tag or namespace is written anywhere in the code, and no official XBRL instance document is produced. The highest state the UI can display is `READY_PENDING_TAXONOMY`; there is no `QAYD_READY` state, and two static tests — one over the module's backend source, one over the frontend source and every i18n key — fail the build if any string claims otherwise.

**Purely additive schema change.** Migration `20260819120000_add_xbrl_readiness_foundation` creates six new `xbrl_*` tables with zero `ALTER` on any existing table and zero data rows read, modified or reinterpreted; the logical rollback is dropping the six tables in reverse dependency order. `Account` gains one back-relation only — no new column on `accounts`.

**Accounting isolation is structural, not disciplinary.** The package writes exclusively to `xbrl_*` tables, and balances come from the existing `financialService.getTrialBalance` — the same trial balance the Financial Center displays, so there is no second source of truth for the same figure. No GL service call, no period-lock interaction, no effect on Payroll, Employee Compensation, Cheques or Inventory. `__tests__/accounting.isolation.test.ts` scans every source file in the module and fails on any accounting write, GL import or HTTP client added later.

**Validation is a report, not a lock.** Fifteen rules with permanently stable codes (`ACC-*`, `RPT-*`, `MAP-*`, `EXP-*`) run as pure functions over an in-memory dataset — no Prisma, no clock inside the rules. An `ERROR` finding never blocks posting a journal entry, approving an invoice or paying a salary.

**Snapshots are immutable by construction.** The service exposes `create`/`list`/`getById` only and the router carries no `PATCH`/`PUT`/`DELETE` on `/snapshots`. A snapshot freezes company info, reporting context, trial balance, account and statement mappings, the taxonomy code/version/official flag, validation results, the readiness score and a SHA-256 source hash — and closes neither the fiscal year nor any period.

**Scope guard.** The reconciliation merge brought migration files and structural Prisma models only. No feature code from `professional-forms-designer-wip` or `vehicle-insurance-v1` entered `production`: there is no service, route, permission key or screen reading `professional_form_templates`, `printed_cheques`, `vehicle_insurance_policies` or `vehicle_accidents`, and both features remain unreleased on their own branches.

**Phase 2, pending the official specification:** official concept codes and namespaces, authority-imposed validation rules, submission package format, submission mechanism (file upload vs. API), e-signature requirements, and the reference map from a contractor chart of accounts to taxonomy concepts — all deliberately not guessed. Resume point: `backend/src/modules/xbrl/export/exporter.ts` → `XbrlExporter.unavailableReason`.

**Validation:** Backend `tsc --noEmit` ✅ · Frontend `tsc --noEmit` ✅ · `prisma validate` ✅ · `migrate status` 69/69 "Database schema is up to date" ✅ · migration drift zero in both directions ✅ · targeted XBRL + migration + accounting suites 12 files / 153 tests ✅ · frontend XBRL contract test 18 ✅. 131 new tests shipped with the package. Product Owner manual visual review of all five tabs: **completed and approved**, no blocking findings.

### Previous Release — `stable-employee-compensation-batch-printing-v1` (`876f0ff8`, 2026-08-19)

**Employee Compensation — Batch Printing v1** — merge of `feature/employee-compensation-batch-printing-v1` (one commit `78cbab4b`, 18 files: 12 modified, 6 new). Source-only release; no installer build and `package.json` stays at `2026.5.4`.

Adds a «طباعة جماعية» button to the Employee Monthly Entitlements list, behind the module's existing `employeeCompensation.print`. A dialog selects the employee and then **only the years that employee actually has statements in**; the year select is disabled until an employee is chosen and resets when it changes, and the print action is disabled until both are set. Printing emits every existing statement for that year in ascending month order, each immediately followed by its own cash payment voucher — statement → its voucher → next month. Months with no calculation are absent rather than padded, and no document is emitted twice.

**Aggregation only.** No calculation is re-run, no approval or snapshot is touched, and no payroll, accounting or general-ledger row is created or read. Both new backend routes (`GET /employee-compensation/print-index` and `GET /employee-compensation/employees/:employeeId/years/:year/statements`) are pure reads guarded by the module's existing print permission — no new permission key, no Prisma model, no migration, no new dependency.

**Identity with individual printing is structural.** The year-statements route builds each statement with the same `getStatementData` the individual statement route calls, so there is no second data path. The two document templates are reused unmodified, and the voucher reference and both QR payload builders are imported from their original sources. `FormPage` was extracted verbatim from `FormLayout` so the printable `.form-page` node is one component shared by both print paths; header and approval block are passed in as `ReactNode`, keeping profile and branding decisions in `FormLayout`. All fourteen administrative forms render identically — two existing tests were updated for the moved locator only.

**Batch-only page geometry.** Every page in the batch uses the `letterhead` profile regardless of the document's individual profile, since a batch is drawn on one stack of pre-printed company paper. Content starts 50mm (statement) and 30mm (voucher) from the sheet edge, held as absolute values in a single `BATCH_CONTENT_TOP` table from which the CSS is generated; sides and bottom come from `PRINT_PROFILES.letterhead` with no duplicated value. Margins are applied as page padding with `@page` zeroed — the model `FormLayout` already uses for the `ready-paper` profile — so print, Save PDF and the accurate preview share one clearance.

**A month with no cash entitlement** (`cashNet <= 0`, the same condition the individual voucher screen uses) prints its statement but has no voucher; a `.no-print` notice names those months and blocks both print buttons until acknowledged. No voucher is created or modified.

**Release scope was deliberately narrowed.** The work was developed on top of `feature/vehicle-insurance-v1`, whose two commits (`353e71d0`, `4e4e26a5` — Vehicle Insurance Management v1) were not in `production`. Merging that branch as-is would have released a second, unreviewed feature, so the feature commit was rebased `--onto production`; this release contains exactly one commit and `git log production..feature` is empty. Vehicle Insurance Management v1 remains unreleased on its own branch.

**Validation:** Backend `tsc --noEmit` ✅ · Frontend `tsc --noEmit` ✅ · `build:back` ✅ · `build:front` ✅ · Frontend 221 files / 4058 tests ✅ · Backend employee-compensation module 11 files / 282 tests ✅ — all re-run on the rebased tree, since the base changed. 33 new tests (27 frontend, 6 backend). Zero new regressions. Physical print output was not verified here (no desktop session); it rests on the Product Owner's completed manual visual review.

### Previous Release — `stable-employee-compensation-configurable-overtime-rate-v1` (`310fcaa7`, 2026-08-16)
