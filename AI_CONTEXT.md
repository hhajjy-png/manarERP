...
| **Current Merge Commit** | `b875f9eb` (merge of `feature/production-release-2026.5.4` — **Production Release 2026.5.4**. Packages a new self-contained Windows installer carrying **Comprehensive Reports — Monthly Employee Entitlements Report Pack v1** (`91fa744d`), which adds «تقرير مستحقات الموظفين الشهرية» **inside** the Comprehensive Reports centre — no standalone page. Its source is exclusively the Employee Monthly Entitlements module (`EmployeeCompensationCalculation`); it reads **no payroll, no journal entry and no general-ledger row**, and every amount comes from the stored per-month totals and the employee snapshot written at save time, so a historical statement reports what was actually approved rather than what a recalculation would produce today — **no calculation logic in that module was touched**. Sixteen columns, a final totals row, seven executive KPIs and two analytical sections aggregated from the displayed rows themselves (no second query, so a section can never diverge from the table or ignore an active filter). Earnings split into bonuses (`BONUS|GRANT|INCENTIVE`) and allowances (`ALLOWANCE`) matching the module's own Arabic labels, with **“other” derived by subtraction** from the stored `totalOtherEarnings` rather than by summing an enumerated list — so a future earning type surfaces in that column instead of silently vanishing, and the row equation `basic + overtime + bonuses + allowances + other − deductions = net` always closes (verified over all 395 statements in the development database with zero mismatches). Debt repayments are shown as an **explanatory** column already inside `totalDeductions`, never subtracted twice. Filters: year · month · employee · department (on the statement's own snapshot, not the live employee file) · statement status. **No new permission key** — the report reuses the module's existing `employeeCompensation.read` following the `payroll` precedent, so the reports centre cannot become a back door to payroll data. Table tooling (column sort, quick search, coloured status chips) is **opt-in per report**: a report that does not declare `tableTools` renders exactly the markup it rendered before, pinned by a dedicated regression test; and during a quick search the totals row is recomputed from the visible rows and relabelled rather than showing server totals over a filtered set. Print/PDF/Excel go through the untouched shared report engine — repeating column headers, page counter, A4 landscape, and three Excel sheets preserving column order, 3-decimal KWD formatting and the totals row. **No schema change, no migration, no new dependency, no existing report modified**; `migrate status` stays 64/64 and the installer ships the same 64 migrations and 75 models as 2026.5.3. Full validation before packaging: Backend/Frontend/Electron `tsc --noEmit` clean, `prisma validate` clean, `migrate status` 64/64 applied, Backend 201 files/3257 tests (2 pre-existing skipped diagnostic stubs), Frontend 220 files/4030 tests, Electron 26 files/499 tests — all green, every check re-run on `production` after the merge with the merged tree proven byte-identical to the reviewed branch tree. **Zero new regressions**: against the 2026.5.3 baseline (3230/4024/499) the suites grew to 3257 (+27)/4030 (+6)/499 — exactly the counts of the two new test files — with no test moving from pass to fail and no new skip. Packaging audited directly: `app.asar` (679 entries), `win-unpacked` (3,350 files / 469,399,374 bytes) and all 4,072 `Setup.exe` entries enumerated — zero source maps, `__tests__`, `*.test.*`/`*.spec.*`, `.ts`/`.d.ts`/`.tsx`, `.env`/key/cert files, SQLite journals, `.bak` databases, non-Windows Prisma engines or build-machine state files (`gdrive-token.dat`/`device-identity.json`/`sync-metadata.json`/`gdrive-account.json`/`security.json`), and exactly one `.db` shipped. **The known `prepare-backend-deps.js` stale-client defect did not bite this release** — with no schema change since 2026.5.3 both the repo-root and `backend/node_modules` generated clients were already at 75 models — but the shipped client was still verified by extraction from inside `Setup.exe` at 118,928 bytes / 75 models; the script remains unfixed and is still recorded as required follow-up. **Golden Database refreshed** to the current approved development database (SHA-256 `8ecaa3988d8171c6f861468177943fedb9be3427c3f4ef51a219cc4a0f51b585`, 3,686,400 bytes, replacing 2026.5.3's `add54d68…`/4,243,456), verified byte-identical in **four** places (source · `win-unpacked` · extracted from inside `Setup.exe` with `cmp` exit 0 · `golden-manifest.json`), `integrity_check = ok`, 0 foreign-key violations, 78 tables with **all 75 schema models present** (plus 2 legacy non-schema tables and `_prisma_migrations`), 64/64 migrations applied, checked on a byte-identical copy so the shipped file was never opened by a writer. The dev app was running when the release began, holding the database open for write; the Product Owner closed it on request and the database was confirmed lock-free, sidecar-free and unchanged both before and after packaging. 9 files (6 modified, 3 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `b109cfae` (merge of `feature/production-release-2026.5.3` — **Production Release 2026.5.3**. Packages a new self-contained Windows installer carrying everything merged onto `production` since the 2026.5.2 installer: **Employee Compensation — Daily Overtime Ledger & Legal Compliance Engine v1**, **Hourly Detail + Cash Entitlement Statement + KD/hour Display v1**, and **Cash Payment Voucher v1** — all three already merged, tagged and documented individually; `package.json` version `2026.5.2` → `2026.5.3` is the only tracked-file change. **First installer to ship** migrations `20260816120000_add_overtime_day_entry` and `20260817150000_add_earning_line_hours_rate`, the `OvertimeDayEntry` model, and the `CompensationEarningLine.hours`/`.rate` columns — 64 migrations and 75 models against the previous installer's 62 and 74. Full validation before packaging: Backend/Frontend/Electron `tsc --noEmit` clean, `prisma validate` clean, `migrate status` 64/64 applied, Backend 200 files/3230 tests (2 pre-existing skipped diagnostic stubs), Frontend 219 files/4024 tests, Electron 26 files/499 tests — all green, and every check re-run on `production` after the merge with the merged tree proven byte-identical to the reviewed branch tree. **Zero new regressions**: against the 2026.5.2 baseline (3132/3959/499) the suites grew to 3230 (+98)/4024 (+65)/499, with no test moving from pass to fail and no new skip. Packaging audited directly — `app.asar` (679 entries), `win-unpacked` (3,349 files) and all 4,070 `Setup.exe` entries enumerated: zero source maps, tests, `.ts`/`.d.ts`/`.tsx`, `.env`/key/cert files, SQLite journals, `.bak` databases, non-Windows Prisma engines, engine `.tmp` leftovers or build-machine state files, and exactly one `.db` shipped. **The known `prepare-backend-deps.js` stale-client defect reproduced again and was again worked around by hand**: the repo-root generated client sat at 74 models while the freshly generated `backend/node_modules` client carried 75, so the fresh client was copied over the root copy before packaging; the shipped client was then verified by extraction from inside `Setup.exe` at 118,928 bytes / 75 models, matching `schema.prisma` exactly. The script itself was again deliberately left unchanged and remains recorded as required follow-up. Golden Database verified byte-identical in **four** places (source · `win-unpacked` · extracted from inside `Setup.exe` with `cmp` exit 0 · `golden-manifest.json`), `integrity_check = ok`, 0 foreign-key violations, 79 tables, checked on a byte-identical copy so the shipped file was never opened by a writer. The dev app was running when the release began, holding the database open for write and breaking `prisma generate`; the Product Owner closed it on request and the database was confirmed lock-free, sidecar-free and unchanged before packaging. 1 file changed. Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `bd9d5b47` (merge of `feature/ecmp-cash-payment-voucher-v1` — **Employee Compensation — Cash Payment Voucher v1**. Adds a «طباعة سند صرف» / Print Payment Voucher button beside «طباعة الكشف» on the monthly screen, producing a **cash receipt voucher** on the company's official letterhead whose every field comes from the same monthly calculation — no second data entry, no re-typing of employee, month, amounts or line items. **The voucher amount is cash, not the stored net**: `Voucher Cash Amount = netAmount − basicSalarySnapshot`, derived by the very same `deriveCashEntitlement` the statement, the detailed report and the QR payload use — the template contains no arithmetic of its own, guarded by a test that forbids any subtraction inside it. The basic salary is shown for information with `تم تحويله إلى البنك / Transferred to Bank` beside it and stays out of the paid amount: a sheet the employee signs for cash in hand must not carry a salary that reached his bank account days earlier. Worked examples are pinned by tests — 150 basic / 250 stored net ⇒ 100, Suliman 450 / 570 ⇒ 120, and with a 20 deduction ⇒ 80 (the basic subtracted once, since deductions already sit inside `netAmount`). **Cloned from the administrative Payment Voucher, which was not touched**: brand colour `#2b2e83`, title box, blue label cells, `1.2px #b9bccd` borders, the green amount box and the dotted rules were reused, but `forms/PaymentVoucherTemplate.tsx` itself is byte-identical — generalising it to serve two documents would have meant conditional flags altering a live printed document; three tests guard that it stayed unchanged. **A4 portrait, one page, 40mm top and ≥20mm bottom**: the letterhead profile's own margins are the specification, so no extra top spacer is used. Measured on the real print band — 236.9mm available, 227.9mm used, 9.0mm slack — a four-item month (the maximum in the data) stays on one page and a fifth item still fits within the 199.1mm budget; that was achieved by compressing empty space alone, with no field dropped and no type shrunk below readability. **No electronic header or footer** — the sheet already carries the company's printed letterhead and footer, so `blankHeader`, `hideFormNumber`, `hideApprovalSection` and a new opt-in `hideTitleRule` leave only the voucher body plus its verification code; no page number, no “generated by”, no URL. **The existing tafqeet is reused, applied to the cash amount alone**: `lib/tafqeet.amountToWordsKWD` in both Arabic and English — the same helper the administrative voucher and the cheques use, with no new amount-to-words system — and a test proves the stored net and the basic salary are never spelled out. **The QR is deterministic and carries no print date or time**: five fixed lines (voucher reference, employee name, employee code, period, net cash amount), so a reprint encodes the same bytes — a voucher whose code changes when reprinted is worthless as proof; a test rejects any date or time pattern and any `printDate`/`printedAt`/`timestamp`/`generatedAt`, and the template never reads the device clock. The signature block was simplified by owner decision to a centred «المستلم / Received By» above blank space for a handwritten signature, with **no fabricated receipt date** — there is no stored payment record to read one from. **Zero cash blocks the voucher structurally**: at `cashNet ≤ 0` the page refuses before `FormLayout` is even composed (not merely a hidden button) and states so in both languages, with the month-screen button disabled in the same case. **Printing is read-only**: it means neither APPROVED nor PAID nor received nor posted, changes no calculation, and adds **no payment table, status, cashier ledger, payment date field or receipt confirmation** — the signed paper becomes the proof of receipt. No Prisma model, no migration, no new permission key; `migrate status` stays 64/64. One additive shared-file change: an opt-in `hideTitleRule` flag on `FormLayout`, default `false`, so the administrative voucher — which also passes `title=""` but does not set it — prints exactly as before. **No Payroll/Accounting/GL/Expense/cheque/bank-export write and no DB write of any kind**; the merge touches **frontend files only**. Frontend `tsc --noEmit`, backend `tsc --noEmit`, `prisma validate`, `migrate status` 64/64, 28 new voucher tests, 10 affected files / 174 tests (including the administrative-voucher and completeness guards), full frontend 219 files / 4024 tests, backend 3230 tests, `build:front` — all green, re-verified on `production` post-merge. Application version stays `2026.5.2`; no installer rebuild. 9 files (6 modified, 3 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `3f6d0132` (merge of `feature/ecmp-statement-cash-net-v1` — **Employee Compensation — Hourly Detail + Cash Entitlement Statement + KD/hour Display v1**. Three things, one pack. **(1) Earning lines carry hours and rate as typed columns.** Two nullable columns on `CompensationEarningLine` (`hours Float?`, `rate Float?`): "7 hours x 4.000" is two numbers, not a sentence -- they get their own columns, are summed, and their product is checked. Keeping them inside `notes` would force every reader (month screen, statement, detailed report, tests) to re-parse prose, and the first wording change would break all of them silently. `resolveHourlyDetail` in the engine enforces three rules on every caller (UI, API, script): both or neither, both positive, and `roundMoney(hours x rate) === roundMoney(amount)` -- rounding applied to both sides so `3 x 4.335` is not falsely rejected. A purely financial line (expense, bonus) stays `NULL`/`NULL`, never zero, because zero reads as a measurement. **These hours never enter the legal compliance engine**: the single source of truth for overtime hours remains `OvertimeDayEntry` with its dates, so a line carrying 200 hours produces no warning and does not touch the Article 66 limits (2 h/day, 3 days/week, 90 days/year, 180 h/year) -- dateless numbers must not manufacture a compliance verdict, which is precisely what the daily ledger exists to prevent; guarded by engine tests and a structural test on the month screen. **(2) ComplianceBar stops claiming "within limits" over nothing.** With zero `OvertimeDayEntry` rows and zero aggregate overtime lines the bar now shows a neutral "لا توجد ساعات إضافية يومية مسجلة للتحقق" with no green badge and no alarm colour. The risk was concrete: a month can carry a line labelled "إضافي عادي" whose detail says 7 hours, and a green badge above it reads as legal validation of hours that were never checked and cannot be. With real hours present the normal bar returns unchanged. **(3) The statement now reflects how the company actually pays.** Basic salary is **transferred to the bank**; additional entitlements are **handed over in cash**. A single derivation `deriveCashEntitlement` -- called by the statement, the detailed report and the QR payload alike -- yields `cashNet = netAmount - basicSalarySnapshot` and `additional = grossEntitlements - basicSalarySnapshot`. The basic is subtracted **once**: deductions are already inside `netAmount`, so subtracting them again would cut the cash figure twice. The basic-salary row carries `تم تحويله إلى البنك / Transferred to Bank` beneath it, small and grey, on the row itself rather than as a footnote, so the employee reads the amount and its route together when signing for receipt. Final labels: `إجمالي المستحقات الإضافية / Total Additional Entitlements` and `صافي المستحق نقدًا / Net Cash Entitlement`. **No stored gross or net is shown to the user anywhere** -- not in the statement, not in the detailed report, not in print/PDF, not even small as a "reference": a document carrying two large figures, one of which includes a salary already in the bank, invites reading it as cash owed. The stored values stay in the database untouched and are read from there for audit. The QR payload follows the paper and now encodes the cash net, because encoding the stored net would hand a scanner a larger number than the employee signed for. **Display units inside this module only** became `KD` and `hour` (`28.000 د.ك` -> `28.000 KD`, `7 ساعة × 4.000 د.ك` -> `7 hour × 4.000 KD`), implemented by a local `units.ts` that wraps the shared formatter and swaps the suffix alone -- `forms/shared/formStyles`, `lib/format` and `config/modules` are untouched, so invoices, cheques, accounting, payroll and the fourteen administrative forms still render `KWD`/`د.ك` (guarded by test). The project currency remains `KWD`: storage, 3-decimal precision, amounts, rates, formulas, API payloads and field names (`hourlyRate`/`hours`/`amount`/`netAmount`) are unchanged. **Presentation and additive schema only** -- zero change to `basicSalarySnapshot`, `netAmount`, `grossEntitlements`, line amounts, status, `approvedAt`, `createdByName` or the employee master; **no Payroll/Accounting/GL/Expense/cheque/bank-export write**, with the module's source-scanning isolation guard passing 14/14. Additive migration `20260817150000_add_earning_line_hours_rate`: two `ALTER TABLE ... ADD COLUMN` statements only, no `DROP`/`DELETE`/`TRUNCATE`/`RENAME`/`UPDATE`/`NOT NULL`, no table rebuild, no backfill in the migration, no new permission key. Separately, as data (not code), the historical `ONE_TIME_ECMP_IMPORT_2024_2026_V1` batch had `hours`/`rate` filled on 529 lines from the approved scenario file (395 regular @4.000, 97 weekly-rest @6.000, 37 official-holiday @8.000) while 432 purely financial lines stayed `NULL`/`NULL`; totals did not move -- 395 calculations, 961 lines, extra `37,995.000`, gross `104,445.000`, net `104,445.000`, all DRAFT, `OvertimeLine`/`OvertimeDayEntry` = 0. Employee Compensation backend 10 files/276 tests, frontend compensation 5 files/101 tests, isolation guard 14/14, `tsc --noEmit` x2, `prisma validate`, `migrate status` 64/64, `build:back`/`build:front` -- all green, re-verified on `production` post-merge. Application version stays `2026.5.2`; no installer rebuild. 26 files (20 modified, 6 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `0a3fbd55` (merge of `feature/employee-compensation-daily-overtime-ledger-v1` — **Employee Compensation — Daily Overtime Ledger & Legal Compliance Engine v1**. Moves overtime from dateless monthly aggregates to a dated **daily ledger** that is the source of truth for new calculations, so the four Article 66 limits are genuinely verified instead of merely disclosed. New `OvertimeDayEntry` table holds each overtime hour's date and legal type; `OvertimeLine` still carries the money and rate snapshot but its hours are rewritten from `Σ` of its days **inside the same transaction**, so the total cannot drift from its detail. `date` is `YYYY-MM-DD` **text, not `DateTime`**, deliberately: UTC conversion moved a Kuwait (UTC+3) workday a full day at midnight, sliding it across weeks and — at a month edge — across months, which collapses every weekly and annual check. **Legacy months are preserved with no fabricated backfill** — "legacy" is defined by the absence of daily rows, the migration is purely additive, and converting an old month is an explicit confirmed action with a per-type reconciliation notice comparing stored hours against entered days. **Legacy REGULAR hours now count toward the annual 180-hour limit**: the counter previously read dated rows only, so 170 legacy hours plus 20 daily hours displayed "20 / 180" and approved cleanly while the real balance was 190 — a statutory breach passing because the system forgot what it already knew. Their days stay unknown and are never inferred, so `verification: FULL | PARTIAL` — kept **separate** from `compliant`, because one asks "was the data sufficient to check?" and the other "was a breach proven?" — marks such years `PARTIAL`, naming the months and distinguishing what remained verified (180 h/yr) from what did not (90 days/yr · 3 days/wk); the UI badge gains a third "تحقّق غير مكتمل" state rather than claiming "✓ within limits". A pure `engine/overtimeComplianceEngine.ts` (no Prisma, no system clock) checks 2 h/day · 3 days/week · 90 days/year · 180 h/year, the work week starting Sunday by reusing the repository's documented Kuwait convention from `HolidayEngine`; a single query spanning `year-1-12-25 → year+1-01-07` evaluates weeks whole across month and year boundaries while annual counters stay inside the calendar year. **No legal number or multiplier changed.** **Same-day double classification is prevented** — the model previously accepted `REGULAR` + `WEEKLY_REST` + `OFFICIAL_HOLIDAY` on one date, i.e. two multipliers over the same day's hours. **An official holiday coinciding with the weekly rest day remains a documented Legal Ambiguity**: Articles 67/68 and Ministerial Decision 188/2010 settle neither precedence, nor whether both premiums apply, nor how many compensatory days are owed — so the system neither sums nor ranks them; it rejects the dual classification and flags it `legalAmbiguity` as needing a documented company decision. Compensatory rest is tracked per day (`PENDING`/`SCHEDULED`/`TAKEN`), forced `NULL` on `REGULAR` since Article 66 creates no such entitlement, and counted **across the year** so March's entitlement survives opening April; **no deadline was invented** — the source text states none. The Reverse Calculator creates no fabricated dates or hours: hours come from the days the user actually selected (a 13-hour suggestion with 4 hours of selected days saves **4**), and the distribution assistant touches only user-selected rows. Compliance is evaluated while editing through a read-only `preview`, so a violation shows as it is typed. Draft always saves and a violation stays `STATUTORY` — never downgraded to a warning — while approval alone is blocked and lists its dated reasons. **No Payroll/Accounting/GL integration**; the module's source-scanning isolation guard now also forbids `attendance`, `holiday` and `leave` — exactly the models a daily ledger would tempt a later edit to read. Additive migration `20260816120000_add_overtime_day_entry`: one `CREATE TABLE` plus 3 indexes, no `ALTER`/`DROP`/`DELETE`/`UPDATE`/`INSERT`, no backfill, no new permission key. Employee Compensation backend 9 files/263 tests · frontend compensation 2 files/37 tests · isolation guard 14/14 · `tsc --noEmit` ×2 · `prisma validate` · `migrate status` 63/63 — all green, re-verified on `production` post-merge. Application version stays `2026.5.2`; no installer rebuild. 23 files (17 modified, 6 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `74a946c2` (merge of `feature/full-pre-production-audit-polish-v1` — **Production Release 2026.5.2**. Full Pre-Production Audit & Polish v1, packaged into a new self-contained Windows installer; `package.json` version `2026.5.1` → `2026.5.2`. Three fix commits. **Backend** `506bd582`: `invoices.addPayment` read the invoice and computed its over-payment guard before opening the transaction and then wrote `paidAmount` as an absolute value derived from that stale read — two concurrent submissions each created a payment row and a journal entry while one single total was written, leaving `invoice.paidAmount` permanently diverged from `Σ payments.amount`; the read and validation are now inside the transaction. Also: the cheque-conflict message printed an English weekday with no year (`String(Date).slice(0, 10)` on a DateTime column), and the forms print-log stored the raw request body in the audit log with no schema — now a strict Zod schema clipping fields and bounding lengths. **Electron** `aadf4b15`: `app:restart` called `app.exit(0)`, bypassing `before-quit` so the backend child survived — on Windows the parent's exit does not kill it — leaving port 48211 held so the relaunched instance failed with `EADDRINUSE`, on the backup-restore path that uses this channel every time; the backend is now stopped with its exit awaited before `relaunch`. `before-quit` calls `preventDefault` and its only exit was at the end of an unguarded chain, so a throw left the app unquittable — now try/finally with the exit in `finally`. Added a main-process `uncaughtException`/`unhandledRejection` net. `backupScheduler.postToInternal` resolved on any response and on network error, so its `catch` was dead code and 500s/401s/a non-listening backend all logged as successful backups. **Frontend** `f6cc574e`: the create/edit/fast-entry invoice screens used a second totals formula diverging from the server (no per-line rounding, no tax cap) — 3 × 0.3335 across three lines showed 3.002 against a stored 3.003, and any tax-bearing invoice showed an edit-screen total contradicting the stored value and every report; new `computeInvoiceTotals` mirrors `invoices.calc.ts`. Also: the payslip printed the raw `DRAFT` enum in a formal Arabic document, three AI-assistant nav targets pointed at the undefined `/dashboard` route, and Global Search guarded «المركز المالي» with `financial.read`, a key in neither `constants.ts` nor the seed, so the entry never appeared for any user (real key: `statements.read`). 10 new regression tests; no schema change, no migration, no new permission key. **Pre-existing packaging defect found and fixed here:** the Prisma client shipped inside the installer was stale — `prepare-backend-deps.js` overlays the repo-root generated client, which `prisma generate` stops updating once `backend/node_modules` exists — proven by extracting it from the already-shipped 2026.5.1 installer (104,359 bytes, missing `EntitlementsBankStatement`/`expectedReturnDate`/`companyOvertimeBaseRateSnapshot`, so that installer would have failed at runtime on three shipped features). The client was regenerated and the installer rebuilt; the shipped client now carries 74 models matching `schema.prisma` exactly. `prepare-backend-deps.js` was deliberately left unchanged — a source fix outside the reviewed scope, recorded as required follow-up. Backend 3132/Frontend 3959/Electron 499 tests green, `tsc --noEmit` ×3 clean, `prisma validate`/`migrate status` 62/62 — all re-verified on `production` post-merge. Golden Database verified byte-identical in four places (source · `win-unpacked` · inside `Setup.exe` · `golden-manifest.json`). 20 files (17 modified, 3 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `310fcaa7` (merge of `feature/employee-compensation-configurable-overtime-rate-v1` — **Employee Compensation — Configurable Company Overtime Rate Pack v1**. Lets Employee Compensation choose the company overtime hourly rate from the system instead of it being fixed in code, while the Kuwait Labour Law engine stays the single source of legal minimums. New `Company Overtime Rate` concept, kept strictly separate from the law: `policy/companyOvertimePolicy.ts` derives per-type company rates (REGULAR ×1.00 · WEEKLY_REST ×1.50 · OFFICIAL_HOLIDAY ×2.00) from one base rate, versioned independently as `COMPANY_OVERTIME_POLICY_VERSION = MANAR-COP-v1` — never touching `LEGAL_RULES_VERSION = KW-LL-6/2010-v2`. `engine/effectiveOvertimeRate.ts` is the single point resolving `effectiveRate = max(statutoryMinimumRate, companyDerivedRate)`; a below-floor company choice is accepted, floored, and surfaced as an explicit `COMPANY_OVERTIME_RATE_BELOW_STATUTORY` warning, never a silent cut. Storage: the company-wide default lives in the existing `Setting` table (key `employeeCompensation.companyOvertimeBaseRate`) — no new settings mechanism — and changing it touches zero saved calculations, since each month snapshots its own rate (`companyOvertimeBaseRateSnapshot` + `companyOvertimePolicyVersion`) and every overtime line snapshots its statutory/company/effective rates and source. Additive migration `20260816120000_add_company_overtime_rate` (7 nullable columns, hand-written to avoid an unrelated `RedefineTables` rebuild); `NULL` means "no company policy" and is computed with the original pre-pack formula byte-for-byte, so re-saving an old month never moves its amount — proven live against a seeded pre-pack calculation. Reverse overtime now derives hours from the effective rate of the selected type; copying a previous month follows the current default, not the copied month's snapshot, reporting the difference explicitly. The short official statement never exposes any rate; the internal detailed report shows statutory/company/effective side by side. No Payroll/Accounting/GL/Expense write and no Employee mutation — enforced by the module's source-scanning isolation test, extended to cover the new `Setting` access. Verified live end-to-end on an isolated database/backend copy (default → month create → override → approve → edit-after-approval → copy-previous → statutory floor → validation), confirming the dev database and Payroll/Accounting/GL tables were untouched. 36 new backend tests. Backend 3129 tests/Frontend 3952 tests all green pre-merge; `tsc --noEmit` clean and `prisma validate`/`migrate status` clean re-verified post-merge. 27 files (22 modified, 5 new). Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `e8fea376` (merge of `feature/production-release-2026.5.1` — **Production Release 2026.5.1**. Packages a new self-contained Windows installer including everything merged onto `production` since the 2026.5.0 installer: Employee Entitlements Bilingual One-Page Statement Pack v1, Monthly Entitlements Bank Statement Pack v1, Employee Entitlements Leave Management Pack v1, and Privacy Toggle Tier A v1 — all four already merged, tagged, and documented individually; `package.json` version `2026.5.0` → `2026.5.1` is the only additional tracked-file change. Full validation before packaging: Backend/Frontend/Electron `tsc --noEmit` clean, `prisma validate`/`migrate status` clean (61/61 applied), Backend 196 files/3087 tests, Frontend 215 files/3943 tests, Electron 26 files/499 tests — all green. Packaging content directly audited (`app.asar`/`extraResources`, zero leaked dev/test/source-map/secret files). Golden Database verified byte-identical across source, `win-unpacked`, and extracted from inside `Setup.exe`. 1 file changed. Full detail: `PROJECT_STATE.md`'s release ledger.) · Previous merge `7cb1c738` (merge of `feature/privacy-toggle-tier-a-v1` — **Privacy Toggle Tier A v1**. Audit + fix of the top-bar lock button's "hide/show financial numbers" feature. Root cause: the button was never broken — `togglePrivacy()` is a plain boolean flip with no stale closures or memoization — the defect was coverage: `privacyMode` was read in only 3 files out of ~90 that display money, since most amounts went through separate helpers (`money()`, `MoneyText`, `MoneyCell`, `fcCurrency()`, chart formatters, print templates) that never consulted the privacy store. Fix: `MoneyText`/`MoneyCell` in `config/modules.tsx` now read `usePrivacyMode()` and render through `PrivateAmount`'s existing `pm-mask`/`pm-real` dual-span pattern, reusing its exported `buildLevel1Mask()` — one shared mechanism, no parallel system, zero changes to ~60 existing call sites. Print output unaffected (real values always print). Tier B (`money()`/`fcCurrency()` calls outside JSX, chart tooltip/axis formatters, print-preview screens, the AI `ResultCard`) deferred by design, documented explicitly as out of scope. 4 files (2 modified, 2 new).) · Previous merge `90868cb1` (merge of `feature/employee-entitlements-leave-management-ui-v1` — **Employee Entitlements Leave Management Pack v1**. Bundles four packs: wires the pre-existing, previously-unreachable Leave CRUD (create/approve/reject) into the Employee Entitlements page with no new permission key; a print-leave-form shortcut that opens the existing `LeaveRequest` form prefilled from the **selected** approved leave record (not the latest one), no new template; persists `reason` + new `expectedReturnDate` (one additive column) so a leave request can be reprinted with its original data later; and corrects annual-leave balance consumption — a leave now deducts the balance only for days elapsed as of the calculation date (`clipLeaveIntervalToAsOf`), not the full duration on approval, so a future 92-day leave no longer zeroes the balance before it starts. New `overusedLeaveDays` explains a zero balance from over-consumption instead of hiding it, never as a negative amount. Final Settlement unaffected in code — same canonical `computeAt()` engine, its 51 tests re-verified on `production` post-merge. Weekly rest days documented as NOT excluded (no data source; none invented). 16 files (11 modified, 5 new).) |
| **Current Documentation Commit** | `9f848bb6` |
| **Current Stable Tag** | `stable-production-release-2026.5.4` (previous: `stable-production-release-2026.5.3`) |
| **Current Release Date** | 2026-08-18 |
...
| **Total Stable Releases** | 440 (window 2026-06-07 → 2026-08-18) — `git tag -l "stable-*"` count |
