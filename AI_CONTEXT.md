...
| **Current Merge Commit** | `d52d53dc` — **Production Release 2026.5.10** (Desktop Installer). Not a merge: a packaging release whose only changed file is `package.json` (`2026.5.9` → `2026.5.10`), exactly as 2026.5.9 was done. It ships every package merged onto `production` since the 2026.5.9 installer — two code-bearing (**Employee Debt Acknowledgment Form v1**, `814116b4`; and **Receipts Page & Comprehensive Report v1**, `bcd67793`) and three documentation-only. Migrations stay at **71** and Prisma models at **87** with `schema.prisma` untouched across the whole delta. Installer `release/AlManarERP-Setup-2026.5.10.exe`, 138,583,336 bytes, SHA-256 `056fb1f553a74113a805149831295ac1fbd6c5ba9b8d433d67efcf357cdc1141`, self-contained (no runtime prerequisites). Packaged verification: root and asar `package.json` both `2026.5.10`, exe FileVersion `2026.5.10` / ProductVersion `2026.5.10.0`. Packaged smoke on an isolated userData directory: Electron launches, backend listens, the database initialises from the shipped template, the Receipts page renders its 9 KPI cards and seven period presets, the Comprehensive Reports centre lists 19 reports including `receipts`, the report runs with its eight columns and totals row, print preview shows 104 rows plus totals with no horizontal overflow, PDF is 5 pages and Excel returns 200 — and page/report parity matched in all four filter cases **inside the packaged app**. The Golden DB `backend/data/manar.db` (SHA-256 `417272f87cac19dcd17738a163a970015bff7335d8fd223f7f3dd67123f6c4af`) is byte-identical in size, mtime and hash before and after every release gate, with no journal siblings; it was never used for a test or a live run. Previous: `bcd67793` (merge of `feature/receipts-page-v1` — **Receipts Page & Comprehensive Report v1**, two feature commits: `a82796a8` the **Receipts page** and `313c77c0` the **Comprehensive Receipts report**). **Feature release; desktop stays `2026.5.9`** — no version bump, no installer, no electron-builder. Two screens that share ONE financial source of truth: `Payment` rows belonging to active sales invoices, selected through `SALES_INVOICE_ACTIVE` imported from `shared/services/operational.reporting.ts` — the very predicate `getCollections()` uses, so the numbers match the dashboard, the executive centre, the reports centre and the collections analysis **by definition rather than by coincidence**. The governing date is `Payment.date` (the user-entered collection date that also dates the GL entry), never `createdAt`. **The report does not re-implement the page**: it calls the same `receiptsQueryService` (`listAll` + `summary`), and both build their WHERE clause with the same `buildReceiptWhere`, so `receiptsReport.ts` contains not one Prisma query, filter predicate or sum of its own. Parity was proven live on an isolated database across **10 filter combinations — total and count identical in all 10**. **No double counting by construction**: only the `payments` table is read; `journal_entries` (1:1 with each payment), the `invoices.paidAmount` snapshot written in the same transaction, `bank_statement_transactions` and the legacy `transactions` ledger are never touched — asserted by an explicit test in both packages. **Cheques are disclosed, not invented**: the system has no incoming-cheque lifecycle (`Payment` has no status, `clearedDate` or `depositDate`; the `cheques` table is outgoing-only), so every recorded receipt is a confirmed receipt on its recorded date, the UI says so permanently, and the status filter is the **invoice settlement status** (`PAID`/`PARTIAL`) named as such — no "cleared", "in collection" or "returned" was fabricated. `BANK` and `TRANSFER` are summed together only in the "bank transfers" summary card and stay distinct in every row and in the breakdown. The page is guarded by the existing `invoices.read`; the report registers as report key `receipts` in the financial group and is guarded by the existing `reports.read`/`reports.export` — **no new permission key, no new API endpoint, no new report engine, no sidebar entry for the report, no new dependency**. Zero migrations and zero schema change (migration count 72 = 72, `schema.prisma` untouched). 161 new tests (92 backend + 69 frontend); full regression shows zero new failures against the known pre-existing 5 backend + 3 frontend. Product Owner visual review completed and approved. Previous: `2bdc3d8b` (merge of `docs/completed-features-compaction-v1` — **Completed Features Documentation Compaction & Archive v1**, implementation commit `d2f2096f`). **Documentation Maintenance Release only** — no code, schema, migration, database or build change; desktop stays `2026.5.9`. `PROJECT_STATE.md`'s `## Completed Features` was a flat 126-row table of **315,536 bytes** (~54 % of the file) that mixed *what the system does today* with *how a feature was first built* and with *designs since replaced* — dangerous in a document read at the start of every session. It is now **`## Completed Features — Active Reference`: 16,027 bytes, 56 rows across 11 domain groups, current behaviour only**, and `PROJECT_STATE.md` as a whole dropped 590,449 → **290,940 bytes (−50.7 %)**. All **126** original entries were moved **verbatim** into `docs/history/completed-features/` across 4 domain files (hr-payroll-employees 10, finance-invoicing-banking 31, cheques-printing-documents 32, platform-ui-operations 53), each opening with an index table carrying feature name, stable tag, classification, *superseded by* and a content SHA-256. Entries are classified by **current relevance, not age** — A=9 current core, B=7 standing architecture/rules, C=29 recent history, D=22 historical, E=31 superseded, F=28 documented in detail elsewhere — and **no class is deleted**; the class only decides prominence in the active reference. **Zero information loss, proven by hash:** the invariant `original_entries = archived_entries` holds at `126 = 126`, with an identical SHA-256 multiset against the pre-package commit, zero missing, zero foreign, zero duplicates, zero cross-file duplicates, zero truncation and zero historical rows left behind. **Reading rule now stated in the file itself: if the active reference and an archived entry disagree, the active reference wins** — the archive deliberately preserves superseded designs, so never implement from an archived entry without checking the active reference and the code. Every retained statement was re-verified against the repository rather than copied from the old prose; one stale claim was corrected in the process — **Data Import handles 9 entities, not the 7 the old text asserted** — while the original row stays verbatim in the archive. The `## Previous Release` rotation and `PROJECT_MASTER_STATUS.md` were both left untouched. Product Owner review completed and approved. Previous: `448aa78e` (merge of `docs/project-state-rotation-v1` — **PROJECT_STATE Documentation Rotation & Maintenance v1**, commits `7e9536a2` rotation + `27f54b6f` current-state synchronization). **Documentation Maintenance Release only** — no code, schema, migration, database or build change, and the desktop version stays `2026.5.9`. `PROJECT_STATE.md` had grown to 1,520,435 bytes / 7,963 lines with **185** `## Previous Release` entries, far past its own ~15-entry rotation trigger. It is now **586,890 bytes / 1,545 lines**: the current release plus the **15** most recent previous entries, with the **170** older ones (172 release sections in total, including two legacy entries that still carry a `## Latest Release` heading and were preserved verbatim rather than silently renamed) moved into **`docs/history/`** across 4 archive files (~939,135 bytes), indexed by `docs/history/README.md` with the full accounting in `docs/history/ROTATION_MANIFEST_V1.md`. **Nothing was lost or reworded:** every section was hashed with SHA-256 before and after, giving zero missing, zero foreign, zero duplicate and zero truncated sections, with the count invariant `185 = 15 + 170` and the total-section invariant `188 = 16 + 172` both holding; the diff on the live file was **0 insertions / 6,433 deletions**, a pure removal. The follow-up commit then corrected current-state metadata that had gone stale — Production HEAD `95f67a9c` → `f37c9c7f`, Application version `2026.5.5` → **`2026.5.9`**, stable tags, total stable releases 443 → **462**, remote sync — every value re-derived from `git`, `package.json` and `schema.prisma` rather than copied from prose, with the superseded Production HEAD demoted verbatim instead of deleted. `PROJECT_MASTER_STATUS.md` received the **minimum safe** synchronization (+14/−9 lines: current HEAD, stable tags, application version, plus new migration/model/documentation/archive rows) with **no archived release copied into it** — it still contains zero `## Previous Release` sections — and no redesign of its parallel historical structure, which stays a future maintenance concern. **`## Completed Features` was deliberately not touched** (315,536 bytes, ~54 % of the remaining live file); it is deferred to a separate package, *Completed Features Documentation Compaction & Archive v1*. Product Owner review completed and approved. Previous: `57e2c8a1` (merge of `docs/complete-user-manual-v1` — **Complete User Manual v1**, documentation commit `267679dc`). **Documentation Release only.** A complete illustrated Arabic user manual for manarERP documenting Desktop Production **2026.5.9** — the desktop version is NOT changed by this release; the manual documents it. 128 pages (unnumbered cover + 127 content pages), 31 chapters in 6 parts, 75 figures, written for the Manager, the Accountant and the administrative user. Coverage is mechanical rather than asserted: **77/77** routes in `App.tsx` classified, **36/36** sidebar entries, **52** user-facing pages documented and **4** intentionally excluded with a stated reason (`/xbrl-readiness`, an internal tool whose own code disclaims official conformance; `/statements`, a redirect; `/verify/:uuid`, a public QR page outside the in-app workflow; and the two bank redirects that resolve to `/import`), leaving **zero** unclassified pages. Every module is covered, including payroll eligibility and the missing-employees banner, Leave Requests with the editable request date, both Payment Voucher and both Receipt Voucher variants with their «عادي | ورق الشركة» switch, Cheques and cheque calibration including the PROVISIONAL-template print block, Banks, Bank Statement Import, Bank Salary Analytics, Accounting, Equipment, Insurance, the Expiration Center single-source rule, Reports, Printing, Backup/Sync, Permissions, both role workflows, Quick Reference, Troubleshooting, Best Practices and a Glossary. The version-controlled artifact is `docs/user-manual/build/AlManarERP-User-Manual-2026.5.9-AR.pdf` (14,537,933 bytes, SHA-256 `1ad57ee9d0f7fc526498881b955059caf426a761fc9bd1f764eba8c414fa7de0`), verified against the blob actually stored in git and not merely against the working-tree file; `release/` is fully gitignored, so the delivery copy under `release/docs/` is local only and neither `git add -f` nor a `.gitignore` edit was used. The editable source is `docs/user-manual/parts/*.html` plus `assets/manual.css` and `build-manual.cjs`, which assembles the parts, renders the cover in its own zero-margin pass, resolves the table of contents from the PDF's own extracted text through invisible Latin markers, merges and publishes — and fails the build deliberately if any chapter's page cannot be located. Cairo is reused from inside the repository, so zero new font binaries were added. **Golden DB untouched**: `backend/data/manar.db` SHA-256 `2fa6f514dae39d9f750d1d8fece9b78b9b910040876f5ccb0317a1a6968f0808` identical before and after, same mtime, no stray journal files — a direct application of the permanent rule recorded with 2026.5.9 that a live app run must never touch the production database. All screenshots came from an **isolated, fully anonymized copy** outside the repository (33 employees, 17 customers, 3 users, 58 cheques, plus 6,532 free-text cells across 9 tables covering bank-statement descriptions, snapshotted records and the audit log), with an automated check confirming zero surviving original names in any text column; the un-anonymized temporary copy was deleted afterwards. Zero backend, frontend-behavior, permission, business-logic, integration, Prisma or migration changes (migrations stay at 71); no installer was built, `npm run dist` was not run and `package.json` was untouched. Six quality findings are recorded in `docs/user-manual/FINDINGS.md` and **deliberately left unfixed**. Product Owner visual review completed and approved. Previous: `695f63b7` (merge of `feature/receipt-voucher-letterhead-template-v1` — **Receipt Voucher Company Letterhead Template v1**, feature commit `6b37930c`). A new, independent print template for Administrative Forms -> Receipt Voucher, for the company's PRE-PRINTED letterhead stationery. A toolbar switch offers «عادي | ورق الشركة» and **defaults to the standard sheet**, which opens exactly as before — this is ADDITIVE, not a substitution. The letterhead sheet drops the page furniture only: the form header (logo and company name), the voucher-number reference block and its rule, and the whole footer (approval block + QR + its top rule). Everything that is the voucher itself stays: the «سند قبض / RECEIPT VOUCHER» title, every field, the amounts, the payment method, the description, and the Receiver signature — that signature lives inside `ReceiptVoucherTemplate`, so it is voucher content, not sheet furniture. Geometry is enforced by the page box, never by padding: `@page { size: A4; margin: 50mm 15mm 20mm 15mm; }` — top 50mm, bottom 20mm, sides 15mm, the latter being the Receipt Voucher's own existing value (its current rule is `@page { size: A4; margin: 12mm 15mm }`), checked rather than assumed. Content area 180mm x 227mm. Measured with Chromium itself: the injected @page rule matches, the printed root's padding is 0px, the first painted element sits at 0mm from the page-box origin (so content really begins at 50mm with no spacer or blank div), content height is 156.63mm leaving 70.37mm above the 20mm bottom band, and inside the sheet there are zero images (no logo, no QR) and no approval block while the title and Receiver signature are present. Chromium live pagination: all four combinations (standard/letterhead x Arabic/English) are ONE A4 page; nothing was shrunk. `receipt-voucher-letterhead` is a NEW entry only (`selectable: false`, `blankHeader: true`) with zero deleted or modified lines in `printProfiles.ts`, so no existing profile changed — pinned by a regression test over all six profiles and the selectable list. **The Payment Voucher is protected structurally, not by promise**: `ReceiptVoucher` is a standalone page that never renders `FormPage`/`FormLayout` (its root is `.rcv-preview` and it owns its own inline @page rule), so `FormPage.tsx`, `FormLayout.tsx`, `contentOnly` and every payment-voucher file were left untouched, and `payment-voucher-letterhead` still reads 45mm/20mm/15mm — both pinned by tests. The pattern of Payment Voucher Print Fix & Letterhead Template v1 was reused (independent profile driving @page + sheet switch + dropping page furniture) with NO parallel print path: same `printCurrentView`, same printed root, same PDF path; the shared `RECEIPT_VOUCHER_PAGE_SPEC` is untouched, the letterhead mode passing a local copy with its own margins so the accurate preview and PDF match the paper. The standard sheet keeps its @page string verbatim (`12mm 15mm`) rather than deriving it, so it cannot shift by a fraction of a millimetre. No backend, Prisma, migration or database change (migrations stay at 71); no Universal Print Preview, Print Engine, PDF IPC, CSS or other administrative form touched; the voucher's financial data and logic are untouched. Product Owner visual review completed and approved. Previous: `342723c3` (merge of `feature/payment-voucher-print-fix-and-letterhead-template-v1` — **Payment Voucher Print Fix & Letterhead Template v1**, feature commit `55931008`). Two display/print-only changes to Administrative Forms -> Payment Voucher. (1) The English voucher printed on two A4 pages. The cause was MEASURED, not guessed: the live app was loaded in an offscreen Chromium window, paginated by Chromium's own `printToPDF` at the profile's A4 margins, and every block measured under print media at the real 180mm print column. Two facts together — the form had no vertical safety margin at all (Arabic 278.91mm against 280mm available, clearing by 1.09mm), and the payment-method row wraps to a second line in English (16.40mm -> 21.43mm, +5.03mm) because `Cash/Cheque/Bank Transfer/Bank:/Cheque No.:` are longer than their Arabic counterparts inside a 66%-wide cell with `flexWrap: 'wrap'`; every other row is identical to the millimetre. Total 283.94mm, a 3.94mm overflow, hence page two. The fix is one line: `contentTopOffset` 2cm -> 1cm on `AdminPaymentVoucher` — top WHITESPACE, not content; no font, size, side margin, field order or colour changed and nothing was shrunk. After: Arabic 268.91mm (11.09mm headroom), English 273.93mm (6.07mm headroom). (2) A separate company-letterhead sheet was ADDED, not substituted: a new toolbar switch offers «عادي | ورق الشركة» and defaults to the standard sheet, which opens exactly as before. The letterhead sheet drops the header, the footer (approval + QR + its rule) and the title block, leaving the voucher content alone inside a band the page box itself enforces — `@page { size: A4; margin: 45mm 15mm 20mm 15mm; }` — so content starts 45mm from the sheet top, stops 20mm above the bottom, and keeps the voucher's existing 15mm side margins; the browser cannot paint outside that box, so nothing can drift into the pre-printed header or footer bands. Verified by measurement: the injected @page rule matches, the resulting band is 180x232mm, `.form-page` print padding is 0px, and the first painted element sits at 0mm from the page-box origin (content really begins at 45mm). Chromium live pagination: all four combinations (standard/letterhead x Arabic/English) are ONE page. **No existing Print Profile changed** — `payment-voucher-letterhead` is a new `selectable: false` entry at the extension point the file itself declares, with zero deleted or modified lines, pinned by a regression test asserting all five existing profiles' margins and the selectable list. No print engine and no Universal Print Preview: zero CSS files, no `utils/print`, no `printing/`, no `FormHeader`. `contentOnly` on `FormPage`/`FormLayout` is off by default, so every existing form renders byte-identically and no other administrative form changed. No migration and no schema: zero files under `backend/` (migrations stay at 71), and the voucher's data and financial logic are untouched. Product Owner visual review completed and approved. Previous: `9a01a724` (merge of `feature/leave-request-editable-request-date-v1` — **Leave Request Editable Request Date v1**, feature commit `8090ed9d`). The printed leave-request form always wrote the submission date as the *day of printing* (`issueDateStr()`) and never stored it, so reprinting an old request showed a date on which the request was never submitted, with no way to correct it — the field was neither editable nor present in the schema. It is now a user-owned field persisted with the leave record, following the existing `expectedReturnDate` pattern (one nullable column + the standard `DateInput` component; no new date system). A new request starts at today's date as a default, not a constraint, via `newLeaveRequestFields()` — a function, not a constant, so a session left open across midnight does not open tomorrow's dialog on yesterday's date. A saved request shows its stored `requestDate` when reopened, whether reached through the «طباعة نموذج الإجازة» shortcut or through the server's `latestLeave`, and a `requestDateManuallyEdited` guard makes the user's own edit win over that automatic adoption. Printing uses the chosen date. Records predating the package (`requestDate = NULL`) fall back to today's date — literally the previous behaviour — so no existing row changed and no data migration was needed. One additive migration, `20260907120000_add_leave_request_date`: a single `ALTER TABLE "leaves" ADD COLUMN "requestDate" DATETIME;` with zero DROP/DELETE/UPDATE, a pure metadata operation in SQLite. **Migrations 70 → 71** in the repository; the shipped 2026.5.8 installer still carries 70 and is unaffected until a future installer is built. No new Prisma model, permission key, dependency or IPC channel. **Print system: zero visual change** — the only edit inside the templates is the *value* of the submission-date line, same tag and same single space; zero CSS/layout/print-engine files are in the package (verified mechanically against `.css`, `FormLayout`, `formStyles`, `printProfiles`, `/printing/`, `fontRegistry`, `pdf.ipc`, `html.service`), and the new input lives only in the screen-only `no-print` editing panel. `startDate`, `endDate`, `days` and every leave calculation are untouched: `days` stays server-derived via `diffDays`, `status` stays forced to `PENDING`, and `requestDate` is read only in the display `select` while the two calculation reads still pick the two dates alone (pinned by a test). Product Owner visual review completed and approved. Previous: `24ea42ec` (merge of `feature/payroll-eligibility-status-transition-integrity-v1` — **Employee ↔ Payroll Eligibility & Status Transition Integrity v1**, feature commit `722097f7`). Payroll rows are a materialized snapshot: `buildSnapshots` sampled `status = 'ACTIVE'` once at generation time and froze the result, so an employee who was ON_LEAVE at that instant got no row and returning to ACTIVE created nothing — the grid renders rows, not eligibility, so they simply vanished. The rule was also duplicated verbatim in two places (generation and gap detection) with no date boundary. There is now ONE source, `payroll.eligibility.ts`: `status = 'ACTIVE' AND (hireDate IS NULL OR hireDate <= period end)` — a null hireDate never disqualifies anyone — read by BOTH consumers, so what generation creates and what detection reports missing can no longer drift apart (pinned by a test comparing the predicate itself). Detection now also stays silent for periods that have not started, without touching the database. Manual single-employee generation deliberately bypasses the shared rule and carries two Product Owner rulings recorded at the override point: ON_LEAVE is ALLOWED (paid leave) while TERMINATED is REFUSED with `لا يمكن إنشاء راتب لموظف منتهي الخدمة.` — by an explicitly named status guard, never by re-applying the bulk rule, which would have silently revoked the ON_LEAVE exception. The refusal is raised before the transaction opens, so no existing payslip is read, rewritten or deleted. The Salaries page now carries the Generate button inside the missing-employees warning itself. Per-employee locking keeps APPROVED/PAID untouched and `upsert` on the unique key makes duplicates structurally impossible; opening the page (GET) still creates nothing. No migration, no schema change, no change to payroll formulas, overtime, EOS, GL or bank exports. Previous: `e1c5e051` (merge of `feature/remove-employee-vehicle-license-expiry-v1` — **Remove Employee Vehicle License Expiry v1**, feature commit `3fa1cf61`) |
| **Current Documentation Commit** | the release-record `docs:` commit on top of `d52d53dc` (the release/version commit for **Production Release 2026.5.10**) — the desktop stable tag stays on the version commit, not on the documentation commit |
| **Current Stable Tag** | `stable-production-release-2026.5.10` → `d52d53dc` — placed on the release/version commit, the same semantics as `stable-production-release-2026.5.9` → `ab9480d9`. The feature tag `stable-receipts-page-and-report-v1` → `bcd67793` stays where it is |
| **Current Release Date** | 2026-09-10 (Production Release 2026.5.10 — **Desktop Installer Release**) |
| **Application Version** | **`2026.5.10`** — **Production Release 2026.5.10**, a new self-contained Windows installer built from `d52d53dc`. Previous: **`2026.5.9`** — **Production Release 2026.5.9**, a new self-contained Windows installer built from `production` HEAD `9c72e717`, release commit `ab9480d9`, tag `stable-production-release-2026.5.9`. It bundles the three packs merged since the 2026.5.8 installer: Leave Request Editable Request Date v1, Payment Voucher Print Fix & Letterhead Template v1, and Receipt Voucher Company Letterhead Template v1. `package.json` is the only tracked-file change in the release commit. **Migrations 70 -> 71** (`20260907120000_add_leave_request_date`, additive), 87 Prisma models with no schema change, no new permission key, no new dependency. Installer `AlManarERP-Setup-2026.5.9.exe` (132.10 MiB, 138,517,894 bytes, SHA-256 `30743922f57559ab37127bbe443336225c0f80b37207e6ba84219787ff65069e`, NSIS, Windows 10/11 x64, zero external runtime prerequisites); `win-unpacked` 3,404 files / 470,519,253 bytes; `app.asar` 688 entries; FileVersion 2026.5.9 / ProductVersion 2026.5.9.0. Golden Database SHA-256 `b3c38fe70b5c27eede3e7dc92df37362f802fceca0e4fbcabcd0930f0f1d1e52` (4,145,152 bytes) verified byte-identical in source, `win-unpacked` and the seed manifest, with exactly one `.db` in the package. Packaging audit clean: zero `__livetest__`/`__probe__`, `.bak`, source maps, `.ts`/`.d.ts`, `.env`, journal files or measurement artifacts. Packed content was verified by extracting `app.asar` and grepping the 438 JS files for each pack's own markers (`requestDate`, `payment-voucher-letterhead`/`45mm`, `receipt-voucher-letterhead`/`50mm`, `contentOnly`) rather than trusting git history. **Both smoke tests passed this time.** The packaged backend was run from the package itself against an ISOLATED COPY of the shipped database (never the real production DB): it listens on 127.0.0.1:48211, `/api/health` returns ok, `prisma migrate status` on that copy reports 71 migrations and an up-to-date schema, `leaves.requestDate` exists, a live Prisma path answers correctly for both a rejected and an accepted login, and the shipped `error.log` is empty. **The GUI also launched**: five live processes, a real main window handle and title, the backend starting inside it, and the database path resolving to `userData` in production mode (Electron 31.0.0 / Node 20.14.0 / Chromium 126.0.6478.36). **This corrects the environment limitation recorded for 2026.5.8**: the packaged GUI was never broken on this machine — the shell had `ELECTRON_RUN_AS_NODE=1` set, which makes the Electron binary run as Node, exit silently and reject Chromium flags with "bad option". Clearing that variable opens the window immediately; no permanent flag was added and no Electron config changed. A NEW PERMANENT RULE was recorded with this release: any test or measurement that needs the app actually running — Chromium/`printToPDF`, live-test, probe — must never use `backend/data/manar.db` directly, because that file becomes the installer's golden database; it must run against an isolated temporary DB or a disposable copy. Previous: **`2026.5.8`** — **Production Release 2026.5.8**, a new self-contained Windows installer built from `production` HEAD `ef1674e4`, release commit `38414d42`, tag `stable-production-release-2026.5.8`. It bundles everything merged since the 2026.5.7 installer: Bank Salary Analytics — Western Digits Consistency v1, Expiration Center — Single Source of Truth & Data Integrity v1, Remove Employee Vehicle License Expiry v1, and Employee ↔ Payroll Eligibility & Status Transition Integrity v1. `package.json` is the only tracked-file change in the release commit. No new migration (70, unchanged from 2026.5.7), no schema change (87 Prisma models, model set verified identical to `backend/prisma/schema.prisma` by the fail-closed build guard and re-verified by extracting the client from inside `Setup.exe`), no new permission key, no new dependency. Installer `AlManarERP-Setup-2026.5.8.exe` (132.07 MiB, 138,492,514 bytes, SHA-256 `ddeea3c66f7849c9f0a32543add0b17a8beadeed2360be6bba449d59f15b3a10`, NSIS, Windows 10/11 x64, zero external runtime prerequisites); `win-unpacked` 3,384 files / 470,406,179 bytes; `app.asar` 688 entries. Golden Database SHA-256 `dda5d4503ee05b286bbb80ed80cfba292e2f5bc8ab31b72f442a330505edb215` (4,075,520 bytes) verified byte-identical in four places: source · `win-unpacked` · extracted from inside `Setup.exe` · `seed-data/golden-manifest.json`; no build-machine state data shipped. Packaging audit clean: zero `__livetest__`/`__probe__`, zero `.ts`/`.d.ts`, zero source maps, zero real `.env`/`.bak`, zero project test files, exactly one `.db`. **Smoke test is partial and this matters:** the packaged backend was verified from the package itself (listens on 127.0.0.1:48211, `/api/health` returns ok, the shipped Golden Database opens with 70 migrations applied and nothing pending, a live Prisma query path answers correctly, `error.log` empty), but **the packaged GUI could not be started on the build machine at all** — it exits silently under normal launch and under every GPU workaround flag. The differential test settles the cause: the previously installed 2026.5.6, which passed a full GUI smoke test at its own release, now fails identically on the same machine, so the fault is environmental and not a property of 2026.5.8. The main window and the in-app version number were therefore **not** visually verified and must be confirmed on an end-user machine before distribution. This is a regression against 2026.5.7, where `--disable-gpu` still sufficed; no permanent flag was added to the app. Follow-up still open, unchanged here: `scripts/prepare-backend-deps.js` prefers `backend/node_modules/.prisma` as the client source but rewrites that directory at the end of every packaging run, so the next build reads the previous build output — neutralised this release by running `npm run db:generate` first and confirming 87/87 in both sources. Previous: `2026.5.7` — installer `AlManarERP-Setup-2026.5.7.exe` (132.06 MiB, 138,478,346 bytes, SHA-256 `89d4a8c6d10ff705073da2213c8ce43d0538c28848900b695d8d47a463800914`), Golden Database `8ab608cc9416f098d5d510d825b7dfa48cbc26b78d57f4a9220592c6b4b6dc24` (4,059,136 bytes), 70 migrations, 87 models |
...
**Deferred by design (not in progress)**
- **Employee Compensation → Payroll / Accounting Integration Pack** — Employee Compensation v1
  shipped deliberately isolated and posts nothing to payroll, the GL, or expenses. Wiring it into
  the financial cycle is a separate future pack, explicitly out of scope for v1. The reuse surface
  is `backend/src/modules/employee-compensation/engine/` (pure, deterministic, caller-agnostic).
  Nothing in the shipped release is a partial integration.
...
- **Production Release 2026.5.10** (Desktop Installer)
  ...
  since the 2026.5.9 installer. The delta was read from git rather than assumed:
  `ab9480d9..HEAD` is 38 commits across five merges — three documentation-only packages
  (Complete User Manual v1, PROJECT_STATE Rotation & Maintenance v1, Completed Features
  Compaction & Archive v1) and two that carry code: **Employee Debt Acknowledgment
  Administrative Form v1** and **Receipts / Collections Page v1 + Receipts Comprehensive
  Report v1**, both already RELEASED with Product Owner visual approval. Packaging only —
  the sole changed file in the release commit is `package.json`. Migrations stay at 71 and
  Prisma models at 87 with no schema change anywhere in the delta. The installer is
  `release/AlManarERP-Setup-2026.5.10.exe` (138,583,336 bytes, SHA-256 `056fb1f553a74113a805149831295ac1fbd6c5ba9b8d433d67efcf357cdc1141`)
  with no runtime prerequisites on a clean Windows 10/11. Version agrees at every layer:
  root `package.json`, the asar's `package.json`, the packaged exe FileVersion `2026.5.10`
  and ProductVersion `2026.5.10.0`. Smoke-tested from the packaged build itself on an
  isolated userData directory — Electron starts, the backend listens, the database seeds
  from the shipped template, the Receipts page renders nine KPI cards and seven period
  presets, the reports centre lists 19 reports including `receipts`, the report renders its
  eight columns with a totals row, print preview carries 104 rows plus totals with no
  horizontal overflow, PDF is five pages and Excel returns 200 — with page/report parity
  matching in every filter case tested inside the packaged app. The Golden DB was neither
  read for tests nor written: identical hash, size and mtime before and after every gate.

- **Receipts Page & Comprehensive Report v1**
  (2026-09-10, `stable-receipts-page-and-report-v1`) —
  two screens released together that answer "what did we collect?", sharing one financial
  source of truth. **The page** (`#/receipts`, under Financial Management, guarded by the
  existing `invoices.read`) is a daily operational screen: KPI cards for the period total,
  this month versus last month with the delta and a safe percentage, cash / cheques / bank
  transfers, and the largest receipt; a filter bar with seven period presets (the week
  starts **Saturday**, the Kuwaiti working week), customer, receipt method, invoice
  settlement status, a debounced search, amount bounds, removable active-filter chips and a
  reset; a server-sorted, server-paginated table; a details drawer that opens the invoice
  through its existing route; and an Excel export covering every matching row rather than
  the visible page. **The report** registers as report key `receipts` in the Comprehensive
  Reports centre (financial group, guarded by the existing `reports.read`/`reports.export`)
  and inherits the whole existing infrastructure — the standard `/api/reports/:type/preview`
  and `/export` endpoints, the A4-landscape RTL print preview, PDF, and a three-sheet Excel
  workbook — with **no new report engine, no new endpoint and no sidebar entry**.
  Both read `Payment` rows on active sales invoices via `SALES_INVOICE_ACTIVE` imported from
  `operational.reporting.ts`, the same predicate `getCollections()` uses, and both date on
  `Payment.date` rather than `createdAt`. The report calls the page's own
  `receiptsQueryService`, so there is no second financial query anywhere; parity of total and
  count was proven live across ten filter combinations. Only the `payments` table is read —
  the mirrored journal entries, the `paidAmount` snapshot, the bank-statement rows and the
  legacy ledger are never summed with it, which an explicit test enforces in both packages.
  The system has no incoming-cheque lifecycle, so rather than invent one the UI discloses
  that every recorded receipt is confirmed on its recorded date and the status filter is
  clearly labelled the **invoice** settlement status. No migration, no schema change, no new
  permission key, no new dependency, and the desktop version stays `2026.5.9`.
  Product Owner visual review completed and approved.

- **Receipt Voucher Company Letterhead Template v1**
  (2026-09-07, `stable-receipt-voucher-letterhead-template-v1`) —
  an additive company-letterhead print template for the Administrative Receipt Voucher,
  built on the pattern approved for the Payment Voucher. A toolbar switch («عادي | ورق
  الشركة») defaults to the standard sheet, which is unchanged. The letterhead sheet drops
  the page furniture only — form header and logo, the voucher-number reference block, and
  the whole footer including the QR — while keeping the «سند قبض / RECEIPT VOUCHER» title,
  every field and amount, the payment method, the description, and the Receiver signature,
  which is voucher content rather than sheet furniture. Geometry comes from the page box:
  `@page { margin: 50mm 15mm 20mm 15mm }`, a 180x227mm content area, with the first painted
  element measured at 0mm from the page-box origin so content truly starts at 50mm with no
  spacer or padding trick, and 156.63mm of content leaving 70.37mm above the bottom band.
  All four combinations print on one A4 page by live Chromium pagination. The new profile is
  an addition only, no existing profile changed, and the Payment Voucher is protected
  structurally: `ReceiptVoucher` never renders FormPage/FormLayout, so those files and
  `contentOnly` were not touched at all. No backend, Prisma, migration, print-engine or
  other-form changes. Product Owner visual review completed and approved.

- **Payment Voucher Print Fix & Letterhead Template v1**
  (2026-09-07, `stable-payment-voucher-print-fix-and-letterhead-template-v1`) —
  two display/print-only changes to the Administrative Payment Voucher. The English voucher
  printed on two A4 pages; the cause was measured with Chromium itself rather than guessed:
  the form had no vertical safety margin (Arabic cleared 280mm by 1.09mm) and the
  payment-method row wraps to a second line in English (+5.03mm), totalling 283.94mm — a
  3.94mm overflow. Fixed with one line, `contentTopOffset` 2cm -> 1cm: top whitespace, not
  content, with no font, size, margin or field-order change. Both languages now print on one
  page. Separately, a company-letterhead sheet was ADDED alongside the existing one (a toolbar
  switch, standard still the default): no header, no footer, voucher content only, inside
  `@page { margin: 45mm 15mm 20mm 15mm }` so content starts 45mm down and stops 20mm above the
  bottom, enforced by the page box rather than by padding. All four combinations verified as
  one page by live Chromium pagination. No existing Print Profile changed, no print engine or
  Universal Print Preview touched, no other form affected, no migration, and the voucher's
  financial data and logic untouched. Product Owner visual review completed and approved.

- **Leave Request Editable Request Date v1**
  (2026-09-07, `stable-leave-request-editable-request-date-v1`) —
  reported problem: the printed leave-request form stamped «تاريخ تقديم الطلب» with the day of
  printing and stored nothing, so reprinting an older request showed a date on which it was never
  submitted, and nothing could be done about it — the field was not editable and did not exist in
  the schema. Now a user-owned field persisted with the leave record, built on the existing
  `expectedReturnDate` pattern (nullable column + the standard `DateInput`), not a new date system.
  A new request defaults to today; the user may change it before saving; a saved request shows its
  stored `requestDate` on reopen, with a manual-edit guard that beats the automatic adoption; and
  printing uses the chosen value. Rows predating the package (`requestDate = NULL`) still fall back
  to today's date, so the old behaviour is preserved exactly and no data migration was required.
  One additive migration (`20260907120000_add_leave_request_date`, a single ALTER TABLE ADD COLUMN,
  70 → 71). Print system deliberately untouched: only the *value* of that one document line changed
  — zero CSS, layout, sizing, margin, font, coordinate or print-engine edits, verified mechanically
  against the changed-file list; the new input sits in the screen-only `no-print` panel. `startDate`,
  `endDate`, `days` and all leave calculations unchanged. Verified live against a running backend
  (save, read-back, and the real page + real template rendering `تاريخ تقديم الطلب: 02/05/2026` with
  today's date absent from the document). Product Owner visual review completed and approved.

- **Employee ↔ Payroll Eligibility & Status Transition Integrity v1**
  (2026-08-29, `stable-payroll-eligibility-status-transition-integrity-v1`) —
  reported symptom: an employee moved from ON_LEAVE back to ACTIVE saved correctly and showed
  as active on the Employees page, yet never appeared on the Salaries page, so no payslip
  could be approved. Root cause: payroll rows are a frozen materialized snapshot and the grid
  reads rows, not eligibility — nothing re-evaluates status after generation. The eligibility
  rule itself was duplicated in two independent places with no date boundary, producing false
  "missing employee" warnings for months that predate an employee's hireDate and for months
  that have not started, which drowned the one real signal. Now: a single shared predicate
  (`payroll.eligibility.ts`) read by both generation and detection; detection silent for
  periods not yet started; the Generate button placed inside the warning so the remedy sits
  with the finding. Status matrix — ACTIVE: bulk + manual; ON_LEAVE: bulk excluded, manual
  ALLOWED (Product Owner exception for paid leave); TERMINATED: bulk excluded, manual REFUSED
  (`لا يمكن إنشاء راتب لموظف منتهي الخدمة.`) via a named-status guard raised before the
  transaction opens, so no historical payslip is read or altered. APPROVED/PAID protected by
  per-employee locking, duplicates prevented by `upsert` on the unique key, and a GET still
  writes nothing. Verified against a copy of the development database: the reported case
  reproduced exactly (3 ACTIVE employees with no August-2026 row among 25 APPROVED rows), then
  generation created 3 drafts with `skippedLocked = 22` and zero pre-existing rows mutated.
  Product Owner visual review completed and approved. Payroll targeted tests 242/242. No
  migration, no schema change.

- **Remove Employee Vehicle License Expiry v1**
  (2026-08-28, `stable-remove-employee-vehicle-license-expiry-v1`) —
  follow-up to the Single Source of Truth audit, which had flagged this as an open Product
  Owner decision: vehicle registration/licence expiry was recorded on the EMPLOYEE
  (`employee.vehicleLicenseExpiry`) while the owner of that fact is the equipment record
  (`equipment.registrationExpiry`) — two entities tracking the same real-world document, a
  double-count source in the Expiration Center. The Product Owner ruled the equipment record
  canonical. The employees UI no longer uses the field anywhere: table column (and its Excel
  export), add/edit form field, detail drawer (data-driven from `cfg.columns`/`cfg.fields`, so
  it disappeared with them) and the import-template column; `vehiclePlate` stays, it is
  employee data. `EMPLOYEE_VEHICLE_LICENSE` was removed from the Expiration Center in full —
  type union, `CANONICAL_SOURCE` (6 keys now), the employee `select`, the row builder, label,
  icon and filter option — and 6 orphaned i18n keys went with it. `EQUIPMENT_REGISTRATION` is
  the only category mapped to `equipment`, proven by a test where an equipment record and an
  employee expiring on the same day yield exactly one row. Counters are re-derived from the new
  dataset: 23 rows left the centre (expired -1, 30d -1, 90d -1, valid -20), total 137 -> 114,
  `actionable` 11, cards still matching the table under every filter. The DB column is retained
  with all 23 values and marked deprecated in six places so the API contract stays intact — no
  migration, no backfill, no data deletion; the `schema.prisma` change is `///` comments only.
  Operational note: those 23 values are now invisible in every screen until the Product Owner
  decides to delete or migrate them. Two new test files (16 backend + 9 frontend) pin the
  removal; restoring the type temporarily failed 13 backend and 2 frontend tests, proving they
  are not vacuous. 177 backend + 9 frontend targeted tests, both typechecks and the frontend
  build passed.
...
- **Bank Salary Analytics — Western Digits Consistency v1**
  ...
  presentation-only fix on the Bank Salary Analytics page (`BankSalaryAnalytics.tsx`).
  Every integer in the page's tables, KPI/insight cards, summary bar, chart tooltip and
  employee drawer was formatted with `toLocaleString('ar-KW')` in **15 places**, which
  emits Arabic-Indic digits (١١/١٤/١٦/١٧), while amounts and dates already went through
  `lib/format` (locale `en-US`) and rendered Western — so a single table row mixed two
  numeral systems. All 15 call sites now use the existing shared `formatInteger`
  (`frontend/src/lib/format/currency.ts`), the same formatter the money values use: no
  duplicate formatter, no CSS transform, no new locale. Two optional-chaining sites
  (`txData?.meta.total`, `m.employeeCount?`) became explicit guards so a missing value
  still renders `—` instead of `formatInteger(undefined)` printing `0`. Arabic UI, RTL,
  column headers, values/formulas, KWD formatting (`fmt3`/`PrivateAmount`) and dates
  (`formatDate`) are untouched, and no other page or shared file changed — one source
  file, 16+/16−, zero migration, zero schema change, zero API, zero permission key.
  New targeted suite `bankSalaryAnalyticsWesternDigits.test.tsx` (4 tests) renders the
  real page against a mocked API (11/14/16/17) and pins Western digits plus the absence
  of any `U+0660–U+0669` character in **every** table cell, with `dir="rtl"` and the
  Arabic names still in place; reverting the page made 3 of the 4 fail, so the suite is
  not vacuous. Product Owner visual review completed and approved before the merge.
  Frontend pack — no `package.json` change and no installer rebuild.
...
- **Vehicle Insurance Management v1**
  ...
  a self-contained operational module for equipment insurance policies and accident
  history, living in exactly two new tables with a foreign key to `equipment` and
  nothing else: no journal entries, no expenses, no maintenance records, no claims,
  and `equipment.insuranceExpiry` (read by the Document Expiration Center) untouched.
  Renewal creates a new policy rather than mutating the old one and accidents are a
  permanent log, so the module exposes no delete path and deliberately has no
  `delete` or `approve` permission key. Four keys (`vehicleInsurance.read`/`create`/
  `update`/`export`), route `/vehicle-insurance`, sidebar entry, `/api/vehicle-insurance`
  router, a report inside the Reports centre, and 154 i18n keys. **A recovery, not new
  work**: implemented in `353e71d0`, it never reached production because a
  `rebase --onto production` dropped it during the batch-printing release — deliberate
  and documented at the time. The code was never deleted; the original branch was the
  sole source and the twelve package-owned files are byte-identical to it. No new
  migration (`20260818120000` was already applied and already on production from the
  reconciliation pack, and both models were already declared structurally, so nothing
  was duplicated). Seven time-drift conflicts resolved by keeping both sides, with no
  production line replaced by an older version. A stale `routerFutureFlags` assertion
  (64 vs an actual 65) was corrected to 66 in passing. Source-only release.
...
- **Employee Compensation — Batch Printing v1**
  ...
  batch printing of a full year of monthly entitlement statements, each followed
  immediately by its own cash payment voucher, in one print job and one preview.
  Employee → only the years that employee has statements in → statement, its
  voucher, next month, ascending, for the months that actually exist. Aggregation
  and ordering of existing documents only: no calculation re-run, no approval or
  snapshot touched, no payroll/accounting/GL row created or read, and every
  document's individual print path unchanged. Both new backend routes are pure
  reads under the module's existing `employeeCompensation.print` — no new
  permission key, no schema change. `FormPage` was extracted from `FormLayout` so
  the printable `.form-page` node is shared by both print paths and cannot drift.
  Batch pages all use `letterhead`; content starts 50mm (statement) / 30mm
  (voucher) from the sheet edge. Source-only release — no installer build.

- **Employee Compensation — Daily Overtime Ledger & Legal Compliance Engine v1**
  (2026-08-16, `stable-employee-compensation-daily-overtime-ledger-v1`) —
  the Daily Overtime Ledger becomes the source of truth for new calculations,
  so the four Article 66 limits are genuinely verified instead of merely
  disclosed. Legacy months are preserved with no fabricated backfill; their
  REGULAR hours still count toward the annual 180-hour limit, while their
  unknown days force an explicit `PARTIAL` verification state rather than a
  false "within limits". Daily / weekly / annual compliance is checked, with
  weeks evaluated whole across month and year boundaries. Same-day double
  classification is prevented, and a weekly rest day coinciding with an
  official holiday stays a documented Legal Ambiguity that is never
  auto-summed or ranked. Compensatory rest is tracked per day across the year,
  and the Reverse Calculator creates no fabricated dates or hours. No
  Payroll / Accounting / GL integration. No legal number or multiplier
  changed. Released after the Product Owner's manual visual and functional
  review was completed and explicitly approved.
...
- **Docs Backfill Pack v1** (2026-08-14, `stable-docs-master-status-backfill-v1`) —
  ...
  `PROJECT_MASTER_STATUS.md`'s own release-history section (Production Release
  2026.5.0 · Production Release 2026.4.0 · Employee Compensation v1), sourced only
  from `PROJECT_STATE.md` and `AI_CONTEXT.md` with nothing invented, and removes
  the temporary note that had flagged the gap. `PROJECT_MASTER_STATUS.md` is the
  sole file touched; no build or test suite was re-run.

- **Employee Entitlements Bilingual One-Page Statement Pack v1** (2026-08-14,
  `stable-employee-entitlements-bilingual-one-page-v1`) — renames the monthly employee
  entitlements module to «مستحقات الموظف الشهرية» / «Monthly Employee Entitlements» and
  rebuilds its official statement as a **single bilingual A4 page**. Released after the
  Product Owner's manual visual review was completed and explicitly approved.
  Every row prints Arabic and English on one line, Arabic-first RTL with the English half
  isolated in `<bdi dir="ltr">` so parentheses and digits cannot be reordered by the bidi
  algorithm. The document title is one bilingual line, **deliberately identical in `ar` and
  `en`**: it titles the printed sheet, which is bilingual regardless of UI language, so a
  language-dependent heading would print two different titles from one document.
  **No translation is ever invented.** The English half of a *value* comes only from an
  approved source — the stored `Employee.fullNameEn` for the name, the approved job-title
  translation table plus Settings overrides for the job title, and `MONTH_NAMES_EN` against
  the month **already in the calculation record** (never `new Date()`) for the period. A new
  strict `lookupJobTitleEn` returns `null` where the long-standing `getJobTitleEn` falls back
  to the Arabic: that fallback is correct for the English-only forms that depend on it, but on
  a bilingual line it would print «سائق شاحنة / سائق شاحنة» and pass Arabic off as a
  translation. With no approved source the Arabic prints alone, with no dangling separator.
  `statementBilingual.ts` derives all three once and feeds both the sheet and the QR, so the
  two can never disagree.
  **A documented trade-off:** the historical snapshot has no English column, and adding one
  would mean a database migration for a printed line, so the English name is read read-only
  from the employee record. Correcting that name later therefore changes a reprinted old
  statement's English half while the Arabic stays at its snapshot value. The rewritten
  snapshot test proves the other six identity fields still come from the snapshot, and the
  module's source-scanning isolation test already forbids any write to `employee`.
  **One page by layout, not by clipping:** the statement-date row and both former approval
  blocks (`الإقرار والاستلام` in the template, `اعتماد المدير المباشر` in the `FormLayout`
  footer) give way to one horizontal `الاعتماد والاستلام / Approval & Receipt` section holding
  the employee receipt signature and the manager approval side by side. Compact styles are
  **local to this template** — the shared `formStyles` is untouched, so the other fourteen
  administrative forms are byte-identical. No `overflow`, no `transform: scale`, no clipping,
  no paper-size change. The 20mm downward offset uses `FormLayout`'s existing
  `contentTopOffset`, which renders inside the single `.form-page` node that accurate preview,
  Save PDF and print all clone; page padding is zeroed by `@media print` and a first-child
  `margin-top` collapses out of the parent once that padding is zero, so either alternative
  would have made the preview disagree with the sheet.
  **QR narrowed** to employee name · net entitlement · period for this statement alone. The
  optional `payloadLines` short-circuits `formatQrText` *before* the default lines are built,
  so no current or future `QRData` field can leak into a QR whose content was specified; it is
  absent on every other form, whose encoded text is byte-identical and guarded by test. The
  printed reference beneath the code stays the document number, which was never in the payload.
  Also corrects an unscoped `tbody tr:nth-child(even) td` rule in `app/theme.css` that reaches
  every table in the app, printed documents included: it had been shading some statement line
  items but never the totals rows, which already carry an inline background that outranks a
  stylesheet — which is precisely why the shading looked like a property of those rows. Fixed
  inside this template; `theme.css` and every other table are untouched.
  Two additive, opt-in props on shared components (`FormQRCode.payloadLines`,
  `FormLayout.titleFontSize`) default to the previous behaviour. Presentation only: zero
  calculation, business-rule, permission-key, Prisma-model or migration change. 14 files
  (11 modified, 3 new); 28 new frontend contract tests + 3 new backend tests.

- **Employee Compensation v1** (2026-08-12, `stable-employee-compensation-v1`) — a new,
  architecturally isolated module for monthly employee entitlements, released as three
  interlocking packs in one unit (Monthly Compensation v1 · Legal/Accounting Validation +
  UI/UX Corrective Pack · Debt & Advances Ledger v1). Per-employee **annual file** with
  exactly one calculation per employee/year/month (unique constraint), a **historical
  snapshot** written once at creation so later edits to the employee record never alter a
  past statement, and a pure **Kuwait Labour Law overtime engine**: Art. 66 regular
  overtime ×1.25, Art. 67 weekly rest ×1.50 + compensatory day, Art. 68 official holiday
  ×2.00 + compensatory day. Hourly rate = basic ÷ **208** (26 days × 8 h) with the divisor
  **imported** from `DAILY_WAGE_DIVISOR`, so it can never diverge from the project's
  documented daily-wage baseline; `LEGAL_RULES_VERSION = KW-LL-6/2010-v2`. The 180 h/year
  limit is genuinely verified against the database; the 2 h/day, 3 days/week and 90 days/year
  limits are **disclosed as unverified** (no daily timesheet exists) rather than silently
  implied, and a derived monthly ceiling is labelled `DERIVED` and never blocks saving.
  **Reverse overtime** turns a target amount into hours, rounding any part-hour up in the
  employee's favour, and leaves no trace of that method on the signed statement. Approved
  records stay fully editable and deletable — approval is an organisational status, not a
  lock, and there is no reopen workflow. Printing reuses the existing
  FormLayout/PrintWorkspace/`composeStyledFromNode` path: a **short official statement**
  (final line items and amounts only) and an **internal detailed report** (hourly-rate basis,
  multipliers, legal references, reverse metadata, debt balances before/after). The
  **Debt & Advances Ledger** stores no balance at all — `remaining = original − Σ(movements)`
  is always derived; monthly repayments are keyed `(calculationId, debtId)` so editing a
  deduction updates the same movement instead of stacking a second one, deleting a month
  retracts its repayment and the balance rebounds, deleting a debt that carries movements is
  refused explicitly, and a debt persists across years without being copied. Two additive
  migrations, zero rebuild of any pre-existing table. **No payroll, accounting, GL, or expense
  posting and no employee mutation** — enforced by a source-scanning isolation test that also
  forbids merely *reading* thirteen unrelated financial Prisma models.
...
- **Receipts / Collections Page v1 — `RELEASED / COMPLETED`.** Merged in `bcd67793`, tag `stable-receipts-page-and-report-v1`. Product Owner Visual Review — APPROVED.
- **Receipts Comprehensive Report v1 — `RELEASED / COMPLETED`.** Merged in the same commit and tag; shares the page's `Payment` SSoT and query semantics. Product Owner Visual Review — APPROVED.
...
- **`scripts/prepare-backend-deps.js` ships whatever Prisma client the repo root happens to hold** — it
  ...
  The root copy therefore silently stops being refreshed, and the installer ships a stale client. This was
  found during Production Release 2026.5.2 and proven pre-existing: the shipped `AlManarERP-Setup-2026.5.1.exe`
  carries a client generated before migration `20260814120000`, missing `EntitlementsBankStatement`,
  `expectedReturnDate` and `companyOvertimeBaseRateSnapshot` — so that installer would have failed at runtime
  on the Monthly Entitlements Bank Statement, the leave expected-return-date field, and the company overtime
  rate. The 2026.5.2 installer was fixed by regenerating the client by hand before packaging; the script
  itself was deliberately not touched, being outside that release's reviewed scope. It needs its own pack:
  run `prisma generate` as a step of `npm run dist`, or read the client from `backend/node_modules`. Until
  then, every installer's packaged client freshness must be verified by hand against `schema.prisma`.
