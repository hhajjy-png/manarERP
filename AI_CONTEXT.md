...
| **Current Merge Commit** | `ac05c45c` (merge of `feature/cheque-calibration-profiles-v1` — **Simple Cheque Calibration Profiles + Persistent Default v1**, feature commit `9db0051f`). Adds three fixed calibration profiles per bank cheque template — `office` (المكتب), `home` (البيت), `other` (أخرى). The identity of a calibration becomes the pair `bank template + calibration profile`: nine independent `settings` rows for three banks, keyed `cheque.calibration.<bank>-a4.<profile>.v1`, each read and written on its own key, so calibrating Gulf at the office cannot move Gulf at home and neither can touch KFH or NBK. A pair never saved before starts from **its own bank's factory document** — never from another profile's edits and never from another bank. **Legacy compatibility:** a calibration saved before profiles existed lives on the template's un-suffixed key and is inherited by **`home`**, the printer it was actually made on, so the Gulf calibration already in production keeps printing exactly as it did; `office` starts from Gulf factory geometry until calibrated in its own right. Inheritance is strictly a fallback — the pair's own row is looked up first and always wins, whatever the row order. The legacy row is read, never written, never deleted; no migration. **Persistent default:** the chosen profile is saved to one app-wide row, `cheques.defaultCalibrationProfile`, holding one word; the last choice survives closing and reopening the app until the user changes it, and `office` is the first-run value and the fallback for a missing or invalid row, nothing more. The cheques page owns that state and hands it to the Professional Calibration Studio, which is now **controlled** — so page and studio can never disagree, and preview and physical print resolve the **same document**. A batch print mixing bank templates is now refused outright rather than printing every cheque with the first one's geometry. No geometry, field position, offset, print engine, calibration engine, renderer, DB model, API, migration or permission key changed; no `localStorage`. `GULF_BANK` APPROVED · `KFH` and `NBK` PROVISIONAL are untouched, and their production printing stays blocked under every profile. 55 new targeted tests across two suites; frontend typecheck and build clean; full frontend suite 224 files / 4141 tests green; no installer rebuilt. Product Owner visual review completed and approved before merge. Previous merge: `0a015f95` (merge of `feature/multi-bank-cheque-profiles-readiness-v1` — **Multi-Bank Cheque Profiles Readiness v1**, feature commit `972c5aca`). The cheque studio now registers ONE TEMPLATE PER BANK and can open any of them directly, without a real cheque on that bank's account first. Registry: `GULF_BANK` APPROVED, `KFH` and `NBK` PROVISIONAL — bank codes taken from the Multi-Bank registry that already seeds them, so no Bank or BankAccount was created and the account picker is untouched. **Gulf is unchanged**: geometry, placement, field table, Date Block, preview image, calibration key and APPROVED status exactly as released; the only edit to its module is delegating the mm → percent field conversion to the shared converter, which its own suites prove is byte-identical. **KFH and NBK** start from a baseline seeded from the Gulf measurements — the only physical cheque measured so far — and then written out as standalone data that does not read the Gulf profile: cheque 180 × 90 mm at X 117 / Y 60, beneficiary 8 / 23.5, date block 135 / 23 with internal 0 / 9 / 18 mm slots, tafqeet 10 / 33, amount 132 / 40. Independence is structural — each profile is deep-cloned, so the three share no field array, no field, no geometry and no placement, and editing one cannot reach another. Neither uses Gulf's cheque photo (`previewBackground` is null); any image that arrives later stays preview-only and is structurally absent from the printed subtree. The date is ONE block with internal day / month / year slots for every bank, never three designer fields. **They are PROVISIONAL, not APPROVED**, because those numbers are a working baseline rather than measurements of a KFH or NBK cheque and nobody has printed one to check; the printability guard checks status BEFORE geometry, so a complete geometry cannot talk its way past it, and production printing is refused in three places on the cheques page (disabled button, single print, whole batch with the offending cheque numbers named). Studio test print stays available — that is what calibration needs, and it never marks a cheque printed. **One studio for all three**: a template selector reading `BANK_CHEQUE_PROFILES` (the shell keeps no list of its own), and every calibratable template opens the same full studio — designer, drag / resize / rotate / nudge, alignment guides, undo-redo, properties panel, data binding, live preview, save, restore default, test print, global sheet offsets. Each opens its own document, saves to its own key (`cheque.calibration.{gulf-a4,kfh-a4,nbk-a4}.v1`), restores to its own baseline and remounts on the bank code, so edits cannot leak between templates. No new engine, renderer, calibration surface, page, API, DB table or migration; no schema change, no new permission key. Frontend 180 suites green, 126 tests across the targeted suites, typecheck and build clean; no installer rebuilt. Product Owner visual review completed and approved before merge. Previous merge: `2ea4aae8` (merge of `feature/gulf-a4-cheque-template-calibration-v1` — **Gulf Bank A4 Cheque Template + Professional Calibration + Date Block v1**, feature commit `7080d1ed`). Three packages released together, all three RELEASED: the **Gulf Bank A4 cheque template**, its **professional calibration integration**, and the **Date Block rebuild**. The measured Gulf Bank cheque is 180 × 90 mm but no printer feeds that paper, so every cheque now prints on a real A4 landscape page (297 × 210 mm) with the cheque as a virtual area at X 117 / Y 60 mm — right edge flush with the sheet, vertically centred. Field geometry is authored in cheque-local millimetres (beneficiary 8 / 23.5, date block 135 / 23, tafqeet 10 / 33, numeric amount 132 / 40) and the A4 position is always derived, never stored: `final = chequeArea + fieldLocal + calibration`. The app prints four values only — date, beneficiary, numeric amount, tafqeet — and nothing the bank pre-prints; the cheque photo is preview-only and is structurally absent from the printed subtree. **Calibration** is not a surface of its own: the profile is a document handed to the existing professional Cheque Template studio, so it inherits drag, resize, rotate, keyboard nudge, alignment guides, undo/redo, the properties panel, data binding, live preview and test print, plus two global A4 placement offsets that move the cheque area without rewriting any field coordinate. It persists as one `settings` row (`cheque.calibration.gulf-a4.v1`) through the existing `PUT /settings` endpoint — no table, no endpoint, no migration. **The date** was three fields producing three frames and unaligned digits; it is now ONE field (`chequeDate`, 135 / 23 / 28 × 7 mm) whose day, month and year are internal slots at 0 / 9 / 18 mm, printing at 135 / 144 / 153 mm as before. A slot carries only a horizontal offset and width — no y, height, line-height or font — so the digits share one baseline by construction, no slot is selectable, and no group concept remains anywhere. **One approved template:** Classic, the 178 × 89 mm template, the generic A4 template and the database Designer templates were removed from the cheque workflow together with the print-method selector, so Preview, Calibrate and Print act on «قالب شيك الخليج» directly. Nothing was rebuilt underneath: the same Runtime Engine, render surface, print page, Print IPC, tafqeet and KWD money formatter. **Historical data untouched** — no migration and no mass update; the `cheque.template.<bank>` rows, the `cheque_designer_templates` table, the template versions and their backend modules all remain so old cheques stay readable, and `printProfileKey` is unchanged. No schema change, no new permission key. Frontend 178 suites green, 70 tests across the three Gulf suites, both typechecks and both builds clean; no installer rebuilt. Product Owner visual review completed and approved before merge, including the Date Block fix. Previous merge: `ac7cb93d` (merge of `fix/financial-accuracy-unified-v1` — **Financial Accuracy & KPI Integrity v1**, pack commits `e693b1e3` Pack 1 / `b9055f7a` Pack 2 / `d5cb128e` Pack 3 / `71b48b89` Pack 4). Four packs released together after a full read-only audit of every user-visible number in the system. **Pack 1 — confirmed wrong values:** the "Critical +90 days" cards showed `0.000` KWD while `24,586.000` was genuinely aged, because the filter used `dueDate` alone and every invoice carries `dueDate = NULL`, which Prisma's `lt` never matches — it now falls back to `issueDate`, the same convention the receivables-ageing report already used; bank reconciliation workspace totals displayed whole-file figures above a list containing only inserted rows (import #36: `81,939.400` vs `1,500.100`), now split into separate file and stored-row fields; and the monthly invoice report collapsed 70 of 113 invoices — `680,868.000` KWD, 65% of total value — into a single unlabelled period row because `billingYear` is NULL on historically imported invoices, now falling back to `issueDate` for grouping display only, with no stored data modified. Also fixed: reconciliation search and amount filters combined as OR rather than AND; employee search filtered `'active'` against the stored `'ACTIVE'` and so always returned nothing; two of three report cards that had no backend implementation gained one, the third staying hidden pending a product decision. **Pack 2 — naming only, no formula changed:** one Arabic term for "collection rate" had been carrying four distinct formulas across screens, now named Collection Effectiveness (CEI), cumulative, in-scope and period; "total expenses" was a single key for both the approved-only operational metric and the expenses page's all-statuses total; the revenue donut summed only its top five customers under the caption "Total revenue"; Financial Analysis §4 now renders the opening receivables balance that was already in its payload, making the closing-balance equation legible on screen. `docs/financial-kpi-definitions.md` is the written contract, guarded by a label test. **Pack 3 — filters, dates, failure states:** expenses search now reaches the cards as well as the table; the payroll "paid" counter no longer overrides the user's status filter; statements and maintenance moved from UTC-midnight parsing to the project's local-day boundary, which in Kuwait had been dropping the first three hours of the start day and most of the end day; partial profit-and-loss ranges are clamped instead of expanding to whole months; cancelled maintenance no longer counts as due; and a failed request renders an explained dash rather than `0.000` KWD or the previous filter's numbers. **Pack 4 — precision and unification:** five parallel frontend rounding families collapse into `lib/money` mirroring the backend policy; the bank statement module and statement opening balances route through `roundMoney`; expense amounts gain write-time normalisation; three different day-counting implementations become one contract where today stays `0` for the whole local day in Kuwait; repeated thresholds move to `config/thresholds` with purpose-describing names and unchanged values (the two high-debt thresholds stay separate at 5,000 and 10,000 because they serve different cards); user counts read `meta.total` and three dropdowns use `fetchAllRows` instead of truncating silently at 200. A permanent cross-surface parity contract covers receivables, revenue, expenses, collections, +90 ageing, monthly invoices and reconciliation totals, deriving expectations from the reference source rather than restating formulas. Release scope is **73 files, all attributable to Packs 1–4**; unrelated pre-existing work that had been mixed into the development branch was excluded and remains preserved on `fix/financial-accuracy-hotfix-v1`. No schema change, no migration, no new permission key. **Deliberately unchanged, pending product decisions: `OVERDUE` derivation, payroll bank export `APPROVED`/`PAID`, and `POTENTIAL_DUPLICATE` in bank balances.** Backend 3613 · frontend 4157 tests green. Product Owner visual review completed and approved before merge. Previous merge: `a684c66a` (merge of `feature/multi-bank-cheques-foundation-v1` — **Multi-Bank Cheques Foundation v1**, feature commit `c2de9a12`). Bank identity in the cheques module becomes a real `Bank → BankAccount → Cheque` structure; Gulf Bank printing unchanged; new banks are record-ready but not print-enabled. Product Owner visual review completed and approved before merge. Previous merge commit: `0d33b9ea` (merge of `feature/production-release-2026.5.6` — **Production Release 2026.5.6**). A packaging release: it introduces no new feature work, only `package.json` `2026.5.5` → `2026.5.6`, and bundles into a new self-contained Windows installer everything merged onto `production` since the 2026.5.5 installer — **C-1 Critical Accounting Fix Pack v1** (merge `1963201a`), which blocks changing a purchase invoice's `paymentMethod` after GL posting (that edit re-derived the journal's credit account while freezing the settlement state, so Cash got credited twice and Accounts Payable went to a debit balance), and **Packaging Hardening Pack v1** (merge `788c8fe0`), which root-causes the recurring stale-Prisma-client packaging defect (copy source moved to `backend/node_modules` plus a fail-closed model-set guard that ran and confirmed 85/85 in this build) and closes the `__livetest__`/`__probe__` dev-code leak that was verifiably shipping inside the 2026.5.5 installer |
| **Current Documentation Commit** | see `docs:` commit on top of `0a015f95` (previous: `2ea4aae8`) |
| **Current Stable Tag** | `stable-cheque-calibration-profiles-v1` → `ac05c45c` · checkpoint `checkpoint-cheque-calibration-profiles-v1` → `0211cc23` (production HEAD immediately before the merge). Previous: `stable-multi-bank-cheque-profiles-v1` → `0a015f95` · checkpoint `checkpoint-multi-bank-cheque-profiles-v1` → `ce40ad09`. Earlier: `stable-gulf-a4-cheque-template-v1` → `2ea4aae8` · checkpoint `checkpoint-gulf-a4-cheque-template-v1` → `60b4ada8`. Earlier: `stable-financial-accuracy-kpi-integrity-v1` → `ac7cb93d` · checkpoint `checkpoint-financial-accuracy-kpi-integrity-v1` → `f5b0f14f`. Earlier: `stable-multi-bank-cheques-foundation-v1` → `a684c66a` · checkpoint `checkpoint-multi-bank-cheques-foundation-v1` → `c40d14ef`. Before it: `stable-production-release-2026.5.6` → `0d33b9ea` · checkpoint `checkpoint-production-release-2026.5.6` → `788c8fe0` (previous: `stable-c1-critical-accounting-fix-pack-v1` → `1963201a`) |
| **Current Release Date** | 2026-08-25 |
| **Application Version** | `2026.5.6` — **unchanged by Simple Cheque Calibration Profiles + Persistent Default v1, by Multi-Bank Cheque Profiles Readiness v1, by Gulf Bank A4 Cheque Template v1, by Financial Accuracy & KPI Integrity v1 and by Multi-Bank Cheques Foundation v1** (both backend/frontend packs, no installer rebuild), so the shipped installer predates both and the 70th migration. Installer figures below describe the 2026.5.6 build: new installer build. `package.json` `productName: "Al Manar ERP"`. Installer artifact `AlManarERP-Setup-2026.5.6.exe` (132.02 MiB, 138,436,813 bytes, SHA-256 `2b44d5a677f7f29a78d15f6eac373204d56cc928d967fd5f8285e8388bb6e44e`, Windows 10/11 x64, zero external runtime prerequisites); `win-unpacked` 3,376 files / 470,059,994 bytes. Golden Database SHA-256 `03fb8e5e18cef47e04a19c7d80e06359779aefd6c402d9d821ffd5249690f880` (3,870,720 bytes, `integrity_check = ok`, 0 foreign-key violations, 86 tables, 69 applied migrations) verified byte-identical in four places (source · `win-unpacked` · inside `Setup.exe` · `seed-data/golden-manifest.json`). The packaged Prisma client carries all **85** models, model set identical to `backend/prisma/schema.prisma` — confirmed both by the new fail-closed build guard and by extracting the client from inside `Setup.exe`. Packaging audit over 4,107 `Setup.exe` entries and 686 `app.asar` entries: zero source maps, tests, `.ts`/`.d.ts`, `.env`, `.bak`, journals, secrets, non-Windows engines — and, for the first time, **zero `__livetest__`/`__probe__` dev files**, which the 2026.5.5 installer verifiably shipped |
