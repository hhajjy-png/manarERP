...
| **Current Merge Commit** | `e1c5e051` (merge of `feature/remove-employee-vehicle-license-expiry-v1` — **Remove Employee Vehicle License Expiry v1**, feature commit `3fa1cf61`). Vehicle expiry is owned by the equipment record, not the employee record: `employee.vehicleLicenseExpiry` is no longer used anywhere in the employees UI (table column and its export, add/edit form field, detail drawer, import-template column) and the `EMPLOYEE_VEHICLE_LICENSE` type is gone from the Expiration Center entirely — union, `CANONICAL_SOURCE` (now 6 keys), the employee `select`, the row builder, the label, the icon and the filter option, plus 6 orphaned i18n keys. `EQUIPMENT_REGISTRATION` is now the only category whose `sourceModule` is `equipment`, so an equipment record and an employee expiring on the same day produce one row, not two. KPI counters are re-derived from the new dataset as before. The DB column is **retained with all its data (23 employees hold a value)** and marked 'Deprecated - vehicle expiry is owned by Equipment/Vehicle registration' in six places (Prisma schema, Zod create/update schema, sort whitelist, import normaliser, import warning, employee alert builder), so the API contract is unbroken while nothing in the UI or the centre consumes it. No migration, no backfill, no data deletion. Previous: `d64b1902` (merge of `feature/expiration-center-single-source-v1` — **Expiration Center — Single Source of Truth & Data Integrity v1**, fix commit `f919b65c`) |
| **Current Documentation Commit** | see `docs:` commit on top of `e1c5e051` (previous: `b19ad39f`) |
| **Current Stable Tag** | `stable-remove-employee-vehicle-license-expiry-v1` → `e1c5e051` · checkpoint `checkpoint-remove-employee-vehicle-license-expiry-v1` → `b19ad39f` (production HEAD immediately before the merge). Previous: `stable-expiration-center-single-source-v1` → `d64b1902` · checkpoint `checkpoint-expiration-center-single-source-v1` → `c5e76ff1` |
