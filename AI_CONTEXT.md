...
| **Current Merge Commit** | `581ad7ee` (merge of `feature/vehicle-insurance-recovery-v1` — **Vehicle Insurance Management v1**). A self-contained operational module for equipment insurance policies and accident history, living in exactly two new tables with a foreign key to `equipment` and nothing else: **no journal entries, no expenses, no maintenance records, no claims**, and `equipment.insuranceExpiry` (read by the Document Expiration Center) untouched. Renewal creates a new policy instead of mutating the old one and accidents are a permanent log, so there is no delete path and no `delete`/`approve` permission key. Four keys: `vehicleInsurance.read`/`create`/`update`/`export`, independent of `equipment.*` and `maintenance.*`; EQUIPMENT_MANAGER owns the module, ACCOUNTANT gets read + export. Route `/vehicle-insurance`, sidebar entry, `/api/vehicle-insurance` router, a report inside the Reports centre, 154 i18n keys. **This was a recovery, not new work**: the package was implemented in `353e71d0` and never reached production because a `rebase --onto production` dropped it during the batch-printing release — a deliberate, documented scope narrowing. The code was never deleted; recovery used the original branch as the sole source and the twelve package-owned files are byte-identical to it. **No new migration** — `20260818120000_add_vehicle_insurance` was already applied and already on production from the reconciliation pack, and both Prisma models were already declared structurally, so nothing was duplicated; the schema stays at 85 models and the migration folder at 69. Source-only release, application version unchanged. Product Owner manual visual and functional review completed and approved) |
| **Current Documentation Commit** | `13791dd1` |
| **Current Stable Tag** | `stable-vehicle-insurance-management-v1` → `581ad7ee` · checkpoint `checkpoint/pre-vehicle-insurance-v1` → `13f5ee47` (production HEAD immediately before the merge) (previous: `stable-xbrl-readiness-foundation-v1`) |
