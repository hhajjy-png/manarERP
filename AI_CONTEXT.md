...
| **Current Merge Commit** | `34e3b6a7` (merge of `feature/bank-explorer-audit-filter-export-v1` — Bank Account Explorer forensic audit of every filter, KPI and export surface: 14 proven defects fixed, 5 critical. Direction (credit − debit) and Category (document signals, six-tier chain: matched entity → reference prefix → bankFeeType → chequeNumber → system flag → anchored text → unclassified) are now two independent dimensions, each in its own module whose input type structurally excludes the other's fields, with a source-scanning test that fails the build on any cross-import or leak. The server's filters were disagreeing with the screen — direction used `credit>0`/`debit>0` (a two-sided row matched both deposits and withdrawals) and the cheque/transfer/fee filters matched two structural columns against a six-tier displayed category — both fixed, direction via exact Prisma field-reference column comparison and category via a 1:1 server port bound to the UI by a shared 36-case golden fixture read by both test suites. Search extended to chequeNumber/matchedRef/transactionId and made neutral to Arabic letter forms without touching storage; amount range made symmetric around zero; inverted ranges rejected explicitly. The ambiguous single «total» (debits + credits) split into totalDebits/totalCredits/netMovement/turnover so every KPI label matches its maths; mixed-currency and potential-duplicate sets disclosed. Export moved to the server so Excel and CSV always cover the **full filtered set** in table order, with RTL, frozen header, auto filter, KWD `#,##0.000` numeric cells and an audit footer. 27 files (14 modified, 13 added); no Prisma/schema/migration change; carries three earlier same-file packs — Direction Logic Fix v1, Classification Enhancement v1, Classification Hardening Pack v1) |
| **Current Documentation Commit** | `1871d7e4` |
| **Current Stable Tag** | `stable-bank-explorer-audit-filter-export-v1` |
| **Current Release Date** | 2026-08-04 |
| **Total Stable Releases** | 404 (window 2026-06-07 → 2026-08-04) |
