...
| **Current Merge Commit** | `274eff82` (merge of `feature/employee-entitlements-bilingual-one-page-v1` — **Employee Entitlements Bilingual One-Page Statement Pack v1**. Renames the monthly employee entitlements module to «مستحقات الموظف الشهرية» / «Monthly Employee Entitlements» and rebuilds its official statement as a single bilingual A4 page. Every row prints Arabic and English on one line, Arabic-first RTL with the English half isolated in `<bdi dir="ltr">`. The English half of a **value** comes only from an approved source — the stored `Employee.fullNameEn` for the name, the approved job-title translation table plus Settings overrides through a new strict `lookupJobTitleEn` that returns `null` instead of echoing the Arabic back as a pseudo-translation, and `MONTH_NAMES_EN` against the month already held in the calculation record for the period — and the Arabic prints alone when no approved source exists, so no translation is ever invented. `statementBilingual.ts` derives all three once and feeds both the sheet and the QR, so the two cannot disagree. The English name is read from the employee record rather than the historical snapshot, which has no English column: a documented trade-off, since correcting that name later changes a reprinted old statement's English half while the Arabic stays at its snapshot value; the read is read-only and enters no total, and the rewritten snapshot test proves the other six identity fields still come from the snapshot. The statement-date row and both former approval blocks (`الإقرار والاستلام` in the template, `اعتماد المدير المباشر` in the `FormLayout` footer) are replaced by one horizontal `الاعتماد والاستلام / Approval & Receipt` section carrying the employee receipt signature and the manager approval side by side; compact styles stay local to this template so the other fourteen administrative forms are byte-identical; no `overflow`, no `transform: scale`, no clipping, no paper-size change. The document sits 20mm lower through `FormLayout`'s existing `contentTopOffset`, which renders inside the single `.form-page` node that accurate preview, Save PDF and print all clone — page padding is zeroed by `@media print` and a first-child `margin-top` collapses out of the parent once that padding is zero, so either alternative would have split preview from sheet. The QR payload for this statement alone is employee name · net entitlement · period, short-circuited before the default lines are built so no current or future `QRData` field can leak into a QR whose content was specified; every other document's encoded text is byte-identical. Also corrects an unscoped `tbody tr:nth-child(even) td` rule in `app/theme.css` that had been shading printed line items but never the totals rows, which already carry an inline background — fixed inside this template, leaving `theme.css` and every other table untouched. Presentation only: zero calculation, business-rule, permission-key, Prisma-model or migration change. 14 files (11 modified, 3 new).) · Previous merge `6ae6536b` (merge of `audit/full-project-audit-2026-08-13` — **Production Release 2026.5.0**. Packages the **Full Project Engineering Audit** (a six-agent parallel review of the whole repository), the **Prisma Schema & Migration Reconciliation Pack v1** and the **Invoice Items Foreign Key Reconciliation Pack v1**. Audit fixes: **RBAC-01 (CRITICAL)** — a demoted `SYSTEM_ADMIN` kept full privileges for up to 12 h because `requirePermission`/`requireRole` short-circuit on `req.user.roleName`, a value baked into the JWT at login and never re-derived, while no code path invalidates sessions on a role change; `roleId`/`roleName` are now read from the live DB row on every request at zero extra query cost. **RACE-01** — seven list loaders (`ResourcePage`, `Salaries` ×3, `Accounting` ×3) let a slow earlier response overwrite a newer one; the `reqIdRef` guard already proven in `Invoices.tsx` now covers all of them. **A11Y-01** keyboard-reachable logout · **SEC-01** rate limit on the public `/api/verify/:uuid` · **SEC-02** `timingSafeEqual` on the internal backup secret · **MONEY-01** KWD rounding in the salaries summary · **MSG-01** a payroll message claiming a journal posting that never happens. **TEST-01** — 26 frontend tests across 8 files had been failing on `production` itself (proven by running them against the stashed tree); all three causes were tests lagging behind the code, no production source was changed, and the frontend suite is now 3837/3837, retiring a baseline carried across two releases. Reconciliation: `migrate diff` had proposed rebuilding three tables forever. Official tooling proved the migration history reproduces the live database exactly, so `schema.prisma` was the drifted side — two `updatedAt` fields regained the `@default(now())` the hand-written migration `20260625100000` had actually created, with zero database change. The last drifted table was `invoice_items`, whose declared `priceId → project_prices` foreign key no migration had ever created because migration `20260617130000` used `ALTER TABLE ADD COLUMN`, which SQLite cannot use to attach a constraint; migration `20260814010000_add_invoice_items_price_fk` adds it with `ON DELETE RESTRICT ON UPDATE CASCADE`, verified with a SHA-256-verified backup beforehand and an unchanged row-content SHA-256 afterwards, and proven enforced at runtime. `migrate diff --from-migrations --to-schema-datamodel` now reports an empty migration. 24 files (23 modified, 1 new migration).) · Previous merge `f8f7a581` (merge of `feature/gdrive-data-safety-pack-v2` — **Production Release 2026.4.0**. Packages Google Drive Data Safety Pack v2 (F-01 manual upload is now decision-driven with a compile-enforced baseline snapshot · F-02 `Promise.race` replaced by real `AbortSignal` cancellation that no longer releases the sync mutex or runtime lock early · F-03 a Golden Manifest guards a newer embedded template against an older cloud copy · F-04 no build-machine state ships inside the installer · F-05 restore marks the database pending-review and blocks BOTH automatic sync directions) and Test Isolation Pack v1 (backend tests physically cannot reach `backend/data/manar.db`: a per-worker sandbox plus a before/after hash guard) into `AlManarERP-Setup-2026.4.0.exe` (131.83 MiB, 138,240,953 bytes, SHA-256 `83c298e3790b8df60b0632ed9f3db92a6e09ba1c5b0a1a09ad0fcf5f1de6b8c9`, Windows 10/11 x64, per-user install under `%AppData%`). Version `2026.3.2` → `2026.4.0`. 27 files (19 modified, 8 new); no schema/migration/permission-key change.) · Previous merge `cbe0734f` (merge of `feature/employee-compensation-v1` — **Employee Compensation v1**. Releases three interlocking packs as one unit: Employee Monthly Compensation v1, Legal/Accounting Validation + UI/UX Corrective Pack, and Employee Compensation Debt & Advances Ledger Pack v1. A new, architecturally isolated module: per-employee annual file, exactly one calculation per employee/year/month, historical snapshot written once at creation, Kuwait Labour Law overtime engine (Art. 66 regular ×1.25 · Art. 67 weekly rest ×1.50 · Art. 68 official holiday ×2.00), hourly rate = basic ÷ 208 (26 × 8, divisor imported from `DAILY_WAGE_DIVISOR` so it cannot diverge from the project's documented daily-wage baseline), reverse overtime (amount → hours, rounded up in the employee's favour), short official statement + internal detailed report on the existing FormLayout/PrintWorkspace path, approved records remain editable and deletable, and a debt & advances ledger whose balance is derived from movements (never stored) with automatic reversal when a month is edited or deleted. Two additive migrations, zero rebuild of any pre-existing table. No payroll, accounting, GL, or expense posting — enforced by a source-scanning isolation test.) |
| **Current Documentation Commit** | `c4b78f33` |
| **Current Stable Tag** | `stable-employee-entitlements-bilingual-one-page-v1` (previous: `stable-production-release-2026.5.0`) |
| **Current Release Date** | 2026-08-14 |
| **Application Version** | `2026.5.0` — **unchanged** by the current release, which is presentation-only and triggered no installer rebuild; the artifact below is still the one shipped with Production Release 2026.5.0. `package.json` `productName: "Al Manar ERP"`. Installer artifact `AlManarERP-Setup-2026.5.0.exe` (131.85 MiB, 138,256,649 bytes, SHA-256 `53d29a4478ee3f9a791f5f4aa8a455e9ad4d49e571dd74787de58910ad1d3489`, Windows 10/11 x64, per-user install under `%AppData%`). Golden Database SHA-256 `977b8ef2f94ddaf813f74000b94e6b24947f66c3b6d40ecac0fae8361bdaf525` (3,309,568 bytes · 59 migration folders · `integrity_check = ok` · `foreign_key_check` empty) — the approved development database **after** the invoice-items foreign-key migration, verified byte-identical in three places (source · `win-unpacked` · extracted from inside `Setup.exe`) and recorded identically in `seed-data/golden-manifest.json` |
| **Total Stable Releases** | 426 (window 2026-06-07 → 2026-08-14) |
