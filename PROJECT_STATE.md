...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `4c58c8d` — Merge Print Polish Batch 1 into production |
| **Merge Commit** | `4c58c8d` — Merge Print Polish Batch 1 into production |
| **Latest stable tag** | `stable-print-polish-batch1-v1` |
...
| Frontend (Vitest) | 27 | 557 | All passing |
...
*Last updated: 2026-06-25 — Print Polish Batch 1 released. HEAD `4c58c8d` (merge commit). Tag `stable-print-polish-batch1-v1`. 830 backend tests (48 files) / 557 frontend tests (27 files). Print Polish Batch 1: `printI18n.ts` (6 Arabic translation fns, 34 tests), InvoiceDesign1 Arabic label fix, `ProfileConfig` export + 5 new optional fields, 3 helper fns (`resolveTablePadding/Width/Justify`), `branding.template.ts` config-driven logo, `html.service.ts` wires profileConfig, Prisma migration adds `verificationUuid` to invoices, `arabicLabels.ts` (backend-only), `verification` module `GET /api/verify/:uuid` (PII-free), `invoices.service` stamps UUID on create, `DocumentVerificationQR.tsx` SVG QR component, `InvoicePreview.tsx` wires QR in engine mode, jsdom + @testing-library/react added as dev deps. 27 files changed, 1330 insertions. Gemini APPROVED ✅.*
