...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `4c8783a` — Merge auto backup enhancements phase 1 |
| **Stable tag** | `stable-auto-backup-phase1-v1` |
...
1. **Financial Safety Test Suite** — Deeper unit tests for accounting/payroll edge cases, force delete gate assertions, transaction rollback scenarios.
2. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
3. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
4. **Inventory Force Delete Phase 3A** — Audit and design pass for Materials, PurchaseOrders, GoodsReceipts; map FK relationships and safe delete order before any implementation.
5. **Inventory Force Delete Phase 3B** — Implementation of Inventory Force Delete based on Phase 3A audit findings.
...
*Last updated: 2026-06-14 — Auto Backup Enhancements Phase 1 released. Full-stack feature: daily WAL-safe auto backups via backend `backupService.create('AUTO')` (`PRAGMA wal_checkpoint(FULL)` before copy); Electron scheduler triggers via runtime-secret internal API (no JWT, no direct DB file access from main process); retention policy (AUTO only, default 30); startup catch-up; dashboard widget (آخر نسخة احتياطية); Settings controls (enabled/time/retention) with live reconfigure on save; AUTO_BACKUP + AUTO_BACKUP_CLEANUP audit logs (userId: null). Gemini APPROVED after WAL fix. 15 files changed, 410 insertions. 101/101 tests ✓, all TS + builds ✓. Feature commit: `d986c07`. Merge commit: `4c8783a`. Tag: `stable-auto-backup-phase1-v1`. Next: Financial Safety Test Suite.*
