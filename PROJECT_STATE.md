...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `90dccc8` — Merge Arabic PDF Chromium fix into production |
| **Merge Commit** | `90dccc8` — Merge Arabic PDF Chromium fix into production |
| **Latest stable tag** | `stable-arabic-pdf-chromium-fix-v1` |
...
|-------|-------|-------|--------|
| Backend (Vitest) | 46 | 728 | All passing |
...
### TypeScript validation (Arabic PDF Chromium Fix — clean run)
...
cd backend && npm test                     ✅ 728/728 (46 files)
...
*Last updated: 2026-06-25 — Arabic PDF Chromium Fix released. HEAD `90dccc8` (merge commit). Tag `stable-arabic-pdf-chromium-fix-v1`. 728 backend tests (46 files) / 516 frontend tests (25 files). PDF export architecture: Financial Center + FinancialReportsTab → Chromium HTML→PDF (`pdf:exportHtml` IPC, hidden BrowserWindow, Cairo base64 embedded font, RTL, A4 landscape); Invoice/Quotation/Executive/ReportPrint → remain on `pdf:export` (DOM capture). Root cause: PDFKit WinAnsiEncoding (Latin-only, no Arabic reshaper). Fix: html.service.ts buildReportHtml() + Electron pdf:exportHtml IPC. 10 files modified, 4 created. pdf.service.ts retained. Gemini APPROVED WITH MINOR NOTES ✅.*
