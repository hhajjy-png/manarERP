...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `6701a9d` — Merge data import phase 3 contracts expenses |
| **Stable tag** | `stable-data-import-phase3-contracts-expenses-v1` |
...
*Last updated: 2026-06-14 — Data Import Phase 3A+3B released. Contracts and expenses Excel import with FK code resolution (`loadCodeToIdMap()`): customerCode→customerId, managerCode→managerId, contractCode→contractId, supplierCode→supplierId. Expenses imported as PENDING only (no accounting side effects). Expense `notes` column does not exist — silently dropped. Existing 5 import entities unchanged. 7 total supported entities. 158/158 tests ✓, backend tsc ✓, frontend tsc ✓, electron tsc ✓, build:back ✓, build:front ✓. Gemini APPROVED. Feature commit: `1bc4dec`. Merge commit: `6701a9d`. Tag: `stable-data-import-phase3-contracts-expenses-v1`.*
