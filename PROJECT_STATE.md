...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `e2abc8f` — Merge Bank Statement Import & Smart Reconciliation Phase 1 into production |
| **Merge Commit** | `e2abc8f` — Merge Bank Statement Import & Smart Reconciliation Phase 1 into production |
| **Latest stable tag** | `stable-bank-statement-import-phase1-v1` |
...
*Last updated: 2026-06-25 — Bank Statement Import & Smart Reconciliation Phase 1 released. HEAD `e2abc8f` (merge commit). Tag `stable-bank-statement-import-phase1-v1`. 1018 backend tests (51 files) / 588 frontend tests (28 files). New module `bankStatementImport` (16 files) + 2 new Prisma models + migration: 7-bank parser (NBK/KFH/Gulf Bank/Boubyan/Warba/Ahli United), 8-strategy smart matcher (confidence 100/90/75/0, 7 data sources), 9-rule validator, bank fee detector (6 types), reconciliation engine (5 statuses + allowed-transitions guard), posting suggestions (4 types, never auto-post), Excel + HTML reports. Frontend: 5-step import wizard + reconciliation workspace. Integrations Center `bank-statement-import` card promoted to available/stable. 4 permissions: read/create/export/reconcile. Security: 10MB cap, Zod bounds, formula rejection, XSS guard, CSP sandbox, attachment-only, AuditLog. Gemini APPROVED WITH MINOR NOTES ✅. Next priority: Cloud Backup Connectors or future integration improvements.*
