...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `a3f2797` — Merge financial safety test suite phase 1 |
| **Stable tag** | `stable-financial-safety-tests-phase1-v1` |
...
1. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
2. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
3. **Inventory Force Delete Phase 3A** — Audit and design pass for Materials, PurchaseOrders, GoodsReceipts; map FK relationships and safe delete order before any implementation.
4. **Inventory Force Delete Phase 3B** — Implementation of Inventory Force Delete based on Phase 3A audit findings.
...
*Last updated: 2026-06-14 — Financial Safety Test Suite Phase 1 released. Backend-only: pure-function extraction into `invoices.calc.ts` (round3 KWD 3dp, computeTotals, nextStatus, overpaymentExceeds) and `inventory.calc.ts` (roundCost 6dp, calcWAC with negative-stock guard + incomingQty=0 division-by-zero guard, sufficientStock). 33 invoice tests + 24 inventory tests. Total: 158/158 ✓. KWD precision fix (round2→round3, 2dp→3dp) applied after Gemini CHANGES_REQUIRED. Gemini APPROVED. Merge commit: `a3f2797`. Tag: `stable-financial-safety-tests-phase1-v1`. Next: Data Import Phase 3.*
