...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `1d092d2` — Merge project prices force delete phase 2c |
| **Stable tag** | `stable-prices-force-delete-phase2c-v1` |
...
1. **Forms Phase 1B** — Leave Request + Salary Advance print views; `Leave` and `PayrollAdvance` models exist, no new DB needed.
2. **Auto Backup Enhancements** — Configurable schedule intervals, backup retention policy, email notification on failure.
3. **Financial Safety Test Suite** — Deeper unit tests for accounting/payroll edge cases, force delete gate assertions, transaction rollback scenarios.
4. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
5. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
6. **Employees Force Delete Audit** — Full relationship audit (payroll, attendance, leave, deductions, allowances, advances) required before implementation; HR sensitivity.
...
*Last updated: 2026-06-13 — Project Prices Force Delete Phase 2C released. Full-stack feature: `GET /prices/:id/force` (always-unblocked dry-run preview) + `DELETE /prices/:id/force` (direct hard delete, no gates) + `ForceDeleteProjectPriceModal.tsx`. SYSTEM_ADMIN only. No FK children — standalone reference table; values copied at contract/invoice creation time. Existing archive (`isArchived: true`) behavior preserved unchanged. First audit trail written in prices module. 101/101 tests ✓, all TS + builds ✓. Feature commit: `56e01b1`. Merge commit: `1d092d2`. Tag: `stable-prices-force-delete-phase2c-v1`. Force delete now supported for Equipment (Phase 1A), Customers (Phase 1B), Suppliers (Phase 2A), Contracts (Phase 2B), and Project Prices (Phase 2C). Next: Forms Phase 1B.*
