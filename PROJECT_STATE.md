...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `a6029e3` — Merge Unified Report Engine Phase 3 into production |
| **Merge Commit** | `a6029e3` — Merge Unified Report Engine Phase 3 into production |
| **Latest stable tag** | `stable-unified-report-engine-phase3-v1` |
...
|-------|-------|-------|--------|
| Backend (Vitest) | 47 | 801 | All passing |
| Frontend (Vitest) | 25 | 516 | All passing |
...
*Last updated: 2026-06-25 — Unified Report Engine Phase 3 released. HEAD `a6029e3` (merge commit). Tag `stable-unified-report-engine-phase3-v1`. 801 backend tests (47 files, +73 from reportEngine.test.ts) / 516 frontend tests (25 files). Report engine architecture: `buildReportHtml(input, options?)` facade → 7 pure template modules (styles, branding, header, footer, table, watermark, summary) + htmlUtils (esc/fmtCell) + printProfiles + brandingLoader. All Financial Center exports now receive Settings-loaded branding. Legacy Reports (`reports.routes.ts`) migrated to `format=html` Chromium pipeline alongside retained `format=pdf` (PDFKit). 12 files created, 4 modified. No schema changes. No migrations. No IPC changes. Gemini APPROVED WITH MINOR NOTES ✅. Known minor notes (non-blocking): hex color validation deferred; company.logo Settings key mapping deferred; PDFKit retained at format=pdf for API-level compatibility.*
