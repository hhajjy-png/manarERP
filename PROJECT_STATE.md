...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `bb56a83` — Merge Payroll Bank Import Phase 1 into production |
| **Merge Commit** | `bb56a83` — Merge Payroll Bank Import Phase 1 into production |
| **Latest stable tag** | `stable-payroll-bank-import-phase1-v1` |
...
| `integrations` | **Complete — Phase 1 Foundation** — 4 REST endpoints: `GET /api/integrations` (`integrations.read`), `GET /api/integrations/:id` (`integrations.read`), `PUT /api/integrations/:id/settings` (`integrations.configure`), `POST /api/integrations/:id/run` (`integrations.run`). Static registry of 6 integrations. Settings stored in existing `Setting` table (key pattern `integrations.<id>.<field>`). Run endpoint always returns `not_implemented` in Phase 1 + writes AuditLog. `payroll-bank-import` card: `targetRoute: '/payroll/bank-import'` — clicking تشغيل navigates to the dedicated import wizard (frontend routing, no backend change). 21 Vitest tests. |
...
*Last updated: 2026-06-25 — Payroll Bank Import Phase 1 released. HEAD `bb56a83` (merge commit). Tag `stable-payroll-bank-import-phase1-v1`. 919 backend tests (50 files) / 588 frontend tests (28 files). New module `payrollBankImport` — 6 supported banks (Gulf Bank, Ahli United, KFH, Warba, Boubyan, NBK), 4-level matching engine, 9 validators, atomic execute via `SalaryPayment` table, 5-step frontend wizard, Excel + HTML reports. Security hardened: formula injection guard, strict ImportReportSchema, CSP sandbox on PDF handler, attachment-only serving. Integrations Center `payroll-bank-import` card now fully operational via `targetRoute` navigation. Next priority: Bank Statement Import + Bank Reconciliation. Gemini APPROVED WITH MINOR NOTES ✅.*
