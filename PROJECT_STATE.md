...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `3f65fe8` — Merge suppliers force delete phase 2a |
| **Stable tag** | `stable-suppliers-force-delete-phase2a-v1` |
...
1. **Contracts Force Delete Phase 2B** — Complete the force delete trilogy for the three main financial entities. Contracts are linked to invoices, expenses, and prices; gate on invoice existence; nullify expense/materialIssue contractId; cascade delete contract documents. Pattern mirrors Customer FD.
2. **Project Prices Force Delete Phase 2C** — Prices are referenced by contracts (optional FK). Low risk. Gate on active contract references. Archive is the primary path.
3. **Forms Phase 1B** — Leave Request + Salary Advance print views; `Leave` and `PayrollAdvance` models exist, no new DB needed.
4. **Auto Backup Enhancements** — Configurable schedule intervals, backup retention policy, email notification on failure.
5. **Financial Safety Test Suite** — Deeper unit tests for accounting/payroll edge cases, force delete gate assertions, transaction rollback scenarios.
6. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
7. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
...
- Force Delete for Employees and Inventory — HR sensitivity + stock consistency risk; defer until confirmed operational need
...
### Force Delete Scope (Phase 1A + 1B + 2A)
...
- **Project Prices** — referenced by contracts; safe to archive, deferred
- **Employees** — linked to payroll, attendance, and leave records; HR sensitivity
- **Inventory** — material movements affect stock levels; deferred
...
*Last updated: 2026-06-13 — Suppliers Force Delete Phase 2A released. Full-stack feature: `GET /suppliers/:id/force` (dry-run preview) + `DELETE /suppliers/:id/force` (three-gate transactional delete) + `ForceDeleteSupplierModal.tsx`. Three-gate protection: (1) Invoice Gate, (2) Posted GoodsReceipt Gate (accounting integrity), (3) Active PurchaseOrder Gate. On clear: nullifies `expenses.supplierId`, deletes CANCELLED POs + DRAFT GRs, deletes supplier. SYSTEM_ADMIN only. 101/101 tests ✓, all TS + builds ✓. Feature commit: `faa3244`. Merge commit: `3f65fe8`. Tag: `stable-suppliers-force-delete-phase2a-v1`. Force delete now supported for Equipment (Phase 1A), Customers (Phase 1B), and Suppliers (Phase 2A). Next: Contracts Force Delete Phase 2B.*
