...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `3002ea4` — Merge contracts force delete phase 2b |
| **Stable tag** | `stable-contracts-force-delete-phase2b-v1` |
...
1. **Project Prices Force Delete Phase 2C** — Prices are referenced by contracts (optional FK). Low risk. Gate on active contract references. Archive is the primary path.
2. **Forms Phase 1B** — Leave Request + Salary Advance print views; `Leave` and `PayrollAdvance` models exist, no new DB needed.
3. **Auto Backup Enhancements** — Configurable schedule intervals, backup retention policy, email notification on failure.
4. **Financial Safety Test Suite** — Deeper unit tests for accounting/payroll edge cases, force delete gate assertions, transaction rollback scenarios.
5. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
6. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
7. **Inventory Force Delete Audit** — Material movements affect stock levels; full relationship audit required before implementation; deferred until confirmed operational need.
...
- Force Delete for Employees — HR sensitivity; defer until confirmed operational need
- Force Delete for Inventory — stock consistency risk; full audit required first
...
### Force Delete Scope (Phase 1A + 1B + 2A + 2B)
...
*Last updated: 2026-06-13 — Contracts Force Delete Phase 2B released. Full-stack feature: `GET /contracts/:id/force` (dry-run preview) + `DELETE /contracts/:id/force` (single invoice-gate transactional delete) + `ForceDeleteContractModal.tsx`. Single-gate protection: Invoice Gate (blocked when `invoices > 0`). On clear: nullifies `expenses.contractId` + `materialIssues.contractId`, deletes contract (ContractDocuments cascade via schema). SYSTEM_ADMIN only. 101/101 tests ✓, all TS + builds ✓. Feature commit: `58ac38a`. Merge commit: `3002ea4`. Tag: `stable-contracts-force-delete-phase2b-v1`. Force delete now supported for Equipment (Phase 1A), Customers (Phase 1B), Suppliers (Phase 2A), and Contracts (Phase 2B). Next: Project Prices Force Delete Phase 2C.*
