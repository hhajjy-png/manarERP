...
|-------|-------|
| **Branch** | `production` |
| **HEAD** | `2f4ca49` — Merge forms phase 1b official templates |
| **Stable tag** | `stable-forms-phase1b-v1` |
...
1. **Auto Backup Enhancements** — Configurable schedule intervals, backup retention policy, email notification on failure.
2. **Financial Safety Test Suite** — Deeper unit tests for accounting/payroll edge cases, force delete gate assertions, transaction rollback scenarios.
3. **Data Import Phase 3** — Additional import entities (contracts, expenses) using the config-driven frontend.
4. **Mobile Companion App** — Read-only dashboard + approval workflows; no confirmed need yet.
5. **Employees Force Delete Audit** — Full relationship audit (payroll, attendance, leave, deductions, allowances, advances) required before implementation; HR sensitivity.
6. **Inventory Force Delete Audit** — Material movements affect stock levels; full relationship audit required before implementation; deferred until confirmed operational need.
...
*Last updated: 2026-06-13 — Forms Phase 1B released. Full-stack feature: 8 official company forms (Salary Certificate, To Whom It May Concern, Leave Request, Return To Work, Salary Advance, Resignation, Employee Warning, Performance Evaluation). Dual print modes: `full-template` (company branding visible) + `letterhead` (`visibility: hidden` on header/footer — preserves physical height on pre-printed paper). Shared template system: `FormLayout`, `FormHeader`, `FormFooter`, `FormQRCode` (offline QR via `qrcode` npm), `ApprovalSection` (direct manager only). Print audit log: `POST /forms/print-log` fire-and-forget. No new DB models, no migration, no new permission keys. 31 files changed, 2368 insertions. 101/101 tests ✓, all TS + builds ✓. Feature commit: `55761e9`. Merge commit: `2f4ca49`. Tag: `stable-forms-phase1b-v1`. Next: Auto Backup Enhancements.*
